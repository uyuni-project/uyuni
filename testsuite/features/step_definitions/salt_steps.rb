...
Given(/^the Salt master can reach "(.*?)"$/) do |minion|
  ...
  CommonLib.repeat_until_timeout(
    timeout: 300,
    retries: 3,
    message: "Master can not communicate with #{minion}",
    report_result: true
  ) do
...
When(/^I stop salt-minion on "(.*?)"$/) do |minion|
  ...
  node.run('systemctl stop salt-minion', check_errors: false) if %w[
    ceos_minion
    ubuntu_minion
    kvm_server
    xen_server
...
When(/^I start salt-minion on "(.*?)"$/) do |minion|
  ...
  node.run('systemctl restart salt-minion', check_errors: false) if %w[
    ceos_minion
    ubuntu_minion
    kvm_server
    xen_server
...
When(/^I wait at most (\d+) seconds until Salt master sees "([^"]*)" as "([^"]*)"$/) do |key_timeout, minion, key_type|
  ...
  CommonLib.repeat_until_timeout(
    timeout: key_timeout.to_i,
    message: "Minion '#{minion}' is not listed among #{key_type} keys on Salt master"
  ) do
    system_name = get_system_name(minion)
...
When(/^I wait until no Salt job is running on "([^"]*)"$/) do |minion|
  ...
  CommonLib.repeat_until_timeout(message: "A Salt job is still running on #{minion}") do
...
Then(/^the PXE boot minion should have been reformatted$/) do
  ...
  raise(ScriptError, 'Minion is intact') unless output.include?('False')
end
...
Given(/^I am authorized as an example user with no roles$/) do
  ...
  random_value = (0...8).map { rand(65..90).chr }
                        .join.downcase
  @username = "testuser #{random_value}"
...
Then(/^I should see "([^"]*)" short hostname$/) do |host|
  ...
  raise(StandardError, "Hostname #{system_name} is not present") unless has_content?(system_name)
end

Then(/^I should not see "([^"]*)" short hostname$/) do |host|
  system_name = get_system_name(host).partition('.').first
  raise(StandardError, "Hostname #{system_name} is present") if has_content?(system_name)
end
...
Then(/^I should see "([^"]*)" in the command output for "([^"]*)"$/) do |text, host|
  ...
  within("pre[id='#{system_name}-results']") do
    raise(StandardError, "Text '#{text}' not found in the results of #{system_name}") unless has_content?(text)
  end
end
...
When(/^I ([^ ]*) the "([^"]*)" formula$/) do |action, formula|
  ...
  if all(:xpath, xpath_query, wait: DEFAULT_TIMEOUT).any?
    raise(StandardError, "xpath: #{xpath_query} not found") unless find(:xpath, xpath_query, wait: DEFAULT_TIMEOUT).click
  else
    ...
    raise(StandardError, "xpath: #{xpath_query} not found") unless all(:xpath, xpath_query, wait: DEFAULT_TIMEOUT).any?
  end
end
...
Then(/^the "([^"]*)" formula should be ([^ ]*)$/) do |formula, state|
  ...
  # DOM refreshes content of chooseFormulas element by accessing it. Then conditions are evaluated properly.
  find('#chooseFormulas')['innerHTML']
  raise(StandardError, "Checkbox is not #{state}") if all(:xpath, xpath_query).any?
...
When(/^I select "([^"]*)" in (.*) field$/) do |value, box|
  select(value, from: FIELD_IDS[box.to_sym])
end
When(/^I enter the local IP address of "([^"]*)" in (.*) field$/) do |host, field|
  ...
  fill_in fieldids[field], with: net_prefix + addresses[host.to_sym]
end
...
When(/^I enter the IP address of "([^"]*)" in (.*) field$/) do |host, field|
  node = get_target(host)
  fill_in FIELD_IDS[field.to_sym], with: node.public_ip
end
...
When(/^I enter the MAC address of "([^"]*)" in (.*) field$/) do |host, field|
  ...
  fill_in FIELD_IDS[field.to_sym], with: "ethernet #{mac}"
end

When(/^I enter the local zone name in (.*) field$/) do |field|
  reverse_net = CommonLib.get_reverse_net($private_net)
  $stdout.puts("#{$private_net} => #{reverse_net}")
  fill_in FIELD_IDS[field.to_sym], with: reverse_net
end

When(/^I enter the local file name in (.*) field$/) do |field|
  reverse_filename = "master/db.#{CommonLib.get_reverse_net($private_net)}"
  $stdout.puts("#{$private_net} => #{reverse_filename}")
  fill_in FIELD_IDS[field.to_sym], with: reverse_filename
end

When(/^I enter the local network in (.*) field$/) do |field|
  fill_in FIELD_IDS[field.to_sym], with: $private_net
end

When(/^I enter the image name in (.*) field$/) do |field|
  name = compute_image_name
  fill_in FIELD_IDS[field.to_sym], with: name
end
...
When(/^I press "Remove Item" in (.*) section$/) do |section|
  sectionids = {
    'first CNAME': 'bind#available_zones#0#records#CNAME#0',
...
When(/^I press minus sign in (.*) section$/) do |section|
  sectionids = { 'third configured zone': 'bind#configured_zones#2', 'third available zone': 'bind#available_zones#2' }
...
Then(/^the pillar data for "([^"]*)" should be "([^"]*)" on "([^"]*)"$/) do |key, value, minion|
  output, _code = CommonLib.pillar_get(key, minion)
  if value == ''
...
Given(/^I try to download "([^"]*)" from channel "([^"]*)"$/) do |rpm, channel|
  ...
  Tempfile.open(rpm) do |tmpfile|
    ...
    begin
      URI.parse(url).open(ssl_verify_mode: OpenSSL::SSL::VERIFY_NONE) do |urlfile|
...
When(/^I see "([^"]*)" fingerprint$/) do |host|
  ...
  fing = output.split("\n")[1].strip!
  raise(StandardError, "Text: #{fing} not found") unless has_content?(fing)
end
...
When(/^I enter "([^"]*)" password$/) do |host|
  raise(StandardError, "#{host} minion password is unknown") unless %w[kvm_server xen_server].include?(host)
...
When(/^I perform a full salt minion cleanup on "([^"]*)"$/) do |host|
  ...
  if host.include? 'ceos'
    ...
    node.run(
      'apt-get --assume-yes remove salt-common salt-minion && apt-get --assume-yes purge salt-common salt-minion && apt-get --assume-yes autoremove',
      check_errors: false
    )
  else
...
When(/^I install a salt pillar top file for "([^"]*)" with target "([^"]*)" on the server$/) do |file, host|
  ...
  script = "base:\n  '#{system_name}':\n    - '#{file}'\n"
  path = CommonLib.generate_temp_file('top.sls', script)
  CommonLib.inject_salt_pillar_file(path, 'top.sls')
  `rm #{path}`
end

When(/^I install a salt pillar file with name "([^"]*)" on the server$/) do |file|
  source = "#{File.dirname(__FILE__)}/../upload_files/#{file}"
  CommonLib.inject_salt_pillar_file(source, file)
end
...
When(/^I delete a salt "([^"]*)" file with name "([^"]*)" on the server$/) do |type, file|
  ...
  raise(StandardError, 'File Deletion failed') unless return_code.zero?
end
...
When(/^I kill remaining Salt jobs on "([^"]*)"$/) do |minion|
  ...
  puts output if output.include?(system_name) && output.include?('Signal 9 sent to job')
end
...
When(/^I set "([^"]*)" as NIC, "([^"]*)" as prefix, "([^"]*)" as branch server name and "([^"]*)" as domain$/) do |nic, prefix, server_name, domain|
  ...
  raise(ScriptError, "Command failed with following output: #{output}") unless return_code.zero?
end
