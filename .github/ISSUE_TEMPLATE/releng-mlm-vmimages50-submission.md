...
- [ ] Report a new issue for autobuild team with the details of the needed projects at the [release-support board](https://gitlab.suse.de/buildops/release-support/-/issues/new).
  - [ ] For every new release, autobuild team will have to create the release projects for both Server and Proxy images using the schema `SUSE:Products:SUSE-Manager-Server:5.0-YYYY-MM:$arch` and `SUSE:Products:SUSE-Manager-Proxy:5.0-YYYY-MM:$arch`, like for example `SUSE:Products:SUSE-Manager-Server:5.0-2024-12:$arch`.
  - [ ] Check with @admd the list of images and their architectures to be released
  - [ ] If there are more images to be provided or if there's the need to release them for a different set of architectures, add this info in the GitLab issue
...
- [ ] Ensure that the `<version>` tag in the `.kiwi` files is set to the last released maintenance update version for SUSE Manager (for example) `<version>5.0.5</version>`. If the version is older, prepare a SR for fixing it.
