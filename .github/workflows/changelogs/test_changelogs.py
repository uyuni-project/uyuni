...
"""
    ...
    monkeypatch.setattr(
        ChangelogValidator, "get_bugzilla_api", lambda self: bugzilla.Bugzilla(url=None)
    )

    return ChangelogValidator(
        base_path, None, None, DEFAULT_LINE_LENGTH, RegexRules(tracker_filename)
    )
...
def test_validate_chlog_for_wrong_pkg(validator, chlog_file):
    ...
    issues = validator.validate(
        ["pkg/path/mypkg.changes.my.feature", "pkg/other/path/file.txt"]
    )
    assert len(issues) == 2, issues_to_str(issues, 2)
    assert any(
        IssueType.WRONG_CHLOG in str(issue) and "mypkg" in str(issue)
        for issue in issues
    )
    assert any(
        IssueType.MISSING_CHLOG in str(issue) and "otherpkg" in str(issue)
        for issue in issues
    )
...
def test_validate_change_in_subdir(validator, base_path):
    ...
    chlog_file.write_text("- This is a changelog entry.\n")
    issues = validator.validate(
        ["pkg/other/otherpkg.changes.my.feature", "pkg/other/path/file.txt"]
    )
    assert not issues, issues_to_str(issues, 0)
