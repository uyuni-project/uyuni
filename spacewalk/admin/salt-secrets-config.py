...
with open("/etc/salt/master.d/susemanager_engine.conf", "w") as f:  #  pylint: disable=unspecified-encoding
    f.write(
...
with open("/etc/salt/master.d/susemanager_db.conf", "w") as f:  #  pylint: disable=unspecified-encoding
    f.write(
        yaml.safe_dump(
...
with open("/etc/salt/master.d/uyuni_roster.conf", "w") as f:  #  pylint: disable=unspecified-encoding
...
with open("/etc/salt/master.d/susemanager-users.txt", "w") as f:  #  pylint: disable=unspecified-encoding
...
if not all(
    ...
    os.system(
        "openssl req -newkey rsa:4096 -x509 -sha256 -days 3650 -nodes -out /etc/salt/pki/api/salt-api.crt -keyout /etc/salt/pki/api/salt-api.key -subj '/CN=localhost'"  #  pylint: disable=line-too-long
    )
    os.chown(
        "/etc/salt/pki/api/salt-api.crt",
