...
/**
 * This class is responsible for the registration process of the systems on behalf of peripheral servers
 * in a hub-peripheral scenario,
 ...
 */
public class SCCProxyRegistrationCreateUpdateSystems implements SCCSystemRegistrationContextHandler {
    private static final Logger LOG = LogManager.getLogger(SCCProxyRegistrationCreateUpdateSystems.class);
    ...
    public void handle(SCCSystemRegistrationContext context) {
        ...
        final List<SCCRegisterSystemJson> pendingRegistrationSystems = context.getProxyRecords().stream()
                .map(e -> Json.GSON.fromJson(e.getSccCreationJson(), SCCRegisterSystemJson.class))
        ...
        for (List<SCCRegisterSystemJson> batch : systemsBatches) {
            try {
                ...
                context.getProxyRegisteredSystems().addAll(response.getSystems());
            }
