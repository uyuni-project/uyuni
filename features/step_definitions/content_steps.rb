...
Then(/^I should see a text like "([^"]*)"$/) do |arg1|
  if !page.has_content?(Regexp.new("#{debrand_string(arg1)}"))
    sleep 1
    fail if !page.has_content?(Regexp.new("#{debrand_string(arg1)}"))
  end
end
...
Then(/^I should see a "([^"]*)" link$/) do |arg1|
  ...
  if link.nil?
    ...
    fail if !first(:link, debrand_string(arg1)).visible?
  else
    fail if !link.visible?
  end
end
...
Then(/^I should not see a "([^"]*)" link$/) do |arg1|
  fail if !page.has_no_link?(arg1)
end

Then(/^I should see a "([^"]*)" button$/) do |arg1|
  fail if !find_button(arg1).visible?
end
...
Then(/^I should see a "([^"]*)" link in element "([^"]*)"$/) do |arg1, arg2|
  within(:xpath, "//div[@id=\"#{arg2}\" or @class=\"#{arg2}\"]") do
    fail if !find_link(debrand_string(arg1)).visible?
  end
end

Then(/^I should not see a "([^"]*)" link in element "([^"]*)"$/) do |arg1, arg2|
  within(:xpath, "//div[@id=\"#{arg2}\" or @class=\"#{arg2}\"]") do
      fail if !has_no_link?(debrand_string(arg1))
  end
end

Then(/^I should see a "([^"]*)" text in element "([^"]*)"$/) do |arg1, arg2|
  within(:xpath, "//div[@id=\"#{arg2}\" or @class=\"#{arg2}\"]") do
    fail if !has_content?(debrand_string(arg1))
  end
end

Then(/^I should see a "([^"]*)" link in "([^"]*)" "([^"]*)"$/) do |arg1, arg2, arg3|
  fail if !page.has_xpath?("//#{arg2}[@id='#{arg3}' or @class='#{arg3}']/a[text()='#{debrand_string(arg1)}']")
end
...
Then(/^Option "([^"]*)" is selected as "([^"]*)"$/) do |arg1, arg2|
  fail if !has_select?(arg2, :selected => arg1)
end
...
Then(/^radio button "([^"]*)" is checked$/) do |arg1|
  fail if !has_checked_field?(arg1)
end
...
Then(/^I should see "([^"]*)" as unchecked$/) do |arg1|
  fail if !has_unchecked_field?(arg1)
end
...
Then(/^I should see "([^"]*)" in field "([^"]*)"$/) do |arg1, arg2|
  fail if !page.has_field?(arg2, :with => arg1)
end

Then(/^I should see a "([^"]*)" element in "([^"]*)" form$/) do |arg1, arg2|
  within(:xpath, "//form[@id=\"#{arg2}\"] | //form[@name=\"#{arg2}\"]") do
    fail if !find_field(arg1, :match => :first).visible?
  end
end

Then(/^I should see a "([^"]*)" editor in "([^"]*)" form$/) do |arg1, arg2|
  within(:xpath, "//form[@id=\"#{arg2}\"] | //form[@name=\"#{arg2}\"]") do
    fail if !page.find("textarea##{arg1}", :visible => false)
    fail if !page.has_css?("##{arg1}-editor")
  end
end
...
Then(/^I should see a Sign Out link$/) do
  fail if !has_xpath?("//a[@href='/rhn/Logout.do']")
end
...
Then(/^I should see (\d+) "([^"]*)" fields in "([^"]*)" form$/) do |count, name, id|
    within(:xpath, "//form[@id=\"#{id}\" or  @name=\"#{id}\"]") do
        fail if !has_field?(name, :count => count.to_i)
    end
end
