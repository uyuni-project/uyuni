...
Feature: Install MLM dependencies on RKE2
  ...
  Scenario: Apply local path provisioner
    When I apply the RKE2 YAML file "$LOCAL_PATH_PROVISIONER_FILE" on "proxy"
    And I set "$LOCAL_PATH_PROVISIONER_STORAGE_CLASS" storage class as default on "proxy"
    And I run "mkdir -p $LOCAL_PATH_PROVISIONER_PATH" on "proxy"
    And I run "restorecon -R -v $LOCAL_PATH_PROVISIONER_PATH" on "proxy"
