...
Then(/^I should see the power is "([^"]*)"$/) do |status|
  within(:xpath, "//*[@for='powerStatus']/..") do
    repeat_until_timeout(message: "power is not #{status}") do
      break if check_text_and_catch_message_request_timeout?(status)
      find(:xpath, '//button[@value="Get status"]').click
    end
    raise "Power status #{status} not found" unless check_text_and_catch_message_request_timeout?(status)
  end
end
