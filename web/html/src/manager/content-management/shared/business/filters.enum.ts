...
import { FilterServerType } from "../type/filter.type";
...
function findFilterMatcherByKey(key: string | null | undefined): FilterMatcherType | Partial<FilterMatcherType> {
    return _find(filterMatchers, entry => entry.key === key) || {};
}

export function getClmFilterDescription(filter: FilterServerType): string {
