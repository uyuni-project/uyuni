...
def create_fake_migration_path(schema_path, new_version, pr_file=None, version=None):
    ...
    for migration_file in files:
        ...
        f = "%04d-%s" % (num, f)
        fakedir = pcom.pop()
        fakedir = "uyuni-reportdb-schema-%s-to-uyuni-reportdb-schema-%s" % (last_version, new_version)
        pcom.append(fakedir)
        pcom.append(f)
...
def run_upgrade(upgrade_script, new_version):
    ...
    command = "export SUMA_TEST_SCHEMA_VERSION=%s; export PERLLIB=%s; %s -y --reportdb" % (
...
def argparser():
    ...
    args.excluded_tables = ["pg_catalog.setval", "public.rhnpackageevr",
                            "public.rhn_pkg_evr_seq", "public.versioninfo"]
    return args
