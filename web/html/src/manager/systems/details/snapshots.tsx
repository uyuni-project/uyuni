...
const Snapshots = ({ serverId }: Props) => {
  ...
  useEffect(() => {
    Network.get<HttpApiResponse<SnapshotInfo> | SnapshotInfo>("/rhn/manager/api/system/getSnapshotInfo?sid=" + serverId)
      .then((response) => {
        if (isHttpApiResponse(response)) {
          if (response.success) {
  ...
  const refreshSnapshots = () => {
    return Network.post<HttpApiResponse<number> | number>("/rhn/manager/api/system/scheduleSnapshotRefresh", {
      sid: Number(serverId),
    })
      .then((response) => {
        if (isHttpApiResponse(response) && !response.success) {
          return Promise.reject(response);
        }
        const actionId = isHttpApiResponse(response) ? getResponseData(response) : response;
        if (actionId === undefined) {
          return Promise.reject(response);
        }
        const redirectUrl = "/rhn/systems/details/history/Event.do?sid=" + serverId + "&aid=" + actionId;
        setMessages(
          MessagesUtils.info(
            <span>
...
const getErrorMessages = (error: JQueryXHR | HttpApiResponse<unknown>) => {
  if (isHttpApiResponse(error) && error.success === false) {
    return MessagesUtils.error(error.message || t("Unable to schedule snapshot refresh."));
  }
  return Network.responseErrorMessage(error as JQueryXHR);
};

const isHttpApiResponse = <T,>(value: unknown): value is HttpApiResponse<T> =>
  value !== null && typeof value === "object" && Object.prototype.hasOwnProperty.call(value, "success");

const getResponseData = <T,>(response: HttpApiResponse<T>) => response.data ?? response.result;
