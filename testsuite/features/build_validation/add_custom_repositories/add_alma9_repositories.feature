...
@alma9_minion
Feature: Add the Alma 9 distribution custom repositories
  In order to use Alma 9 channels with Red Hat "modules"
  ...
  Scenario: Create CLM filters to remove AppStream metadata from Alma 9
  ...
  Scenario: Create a CLM project to remove AppStream metadata from Alma 9
    ...
    And I select "almalinux-9 for x86_64" from "selectedBaseChannel"
    # "almalinux-9-appstream for x86_64" is already checked
