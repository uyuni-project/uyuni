...
Feature: Prepare the branch server for PXE booting
  ...
  Scenario: Parametrize DHCP on the Retail branch server
    ...
    And I press "Add Item" in host reservations section
    And I enter "sle15sp3terminal" in second reserved hostname field
    And I enter the local IP address of "sle15sp3_terminal" in second reserved IP field
    And I enter the MAC address of "sle15sp3_terminal" in second reserved MAC field
    And I click on "Save Formula"
    Then I should see a "Formula saved" text
  ...
  Scenario: Parametrize DNS on the Retail branch server
    ...
    And I press "Add Item" in A section of example.org zone
    And I enter "sle15sp3terminal" in third A name field of example.org zone
    And I enter the local IP address of "sle15sp3_terminal" in third A address field of example.org zone
