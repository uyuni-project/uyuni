...
Then(/^the SLE15 (SP3|SP4) product should be added$/) do |sp_version|
  ...
  match1 = "[I] SLE-Product-SLES15-#{sp_version}-Pool for x86_64 SUSE Linux Enterprise Server 15 #{sp_version} x86_64 [sle-product-sles15-#{sp_version.downcase}-pool-x86_64]"
  raise "Not included:\n #{match1}" unless output.include? match1
  match2 = "[I] SLE-Module-Basesystem15-#{sp_version}-Updates for x86_64 Basesystem Module 15 #{sp_version} x86_64 [sle-module-basesystem15-#{sp_version.downcase}-updates-x86_64]"
  raise "Not included:\n #{match2}" unless output.include? match2
  match3 = "[I] SLE-Module-Server-Applications15-#{sp_version}-Pool for x86_64 Server Applications Module 15 #{sp_version} x86_64 [sle-module-server-applications15-#{sp_version.downcase}-pool-x86_64]"
  raise "Not included:\n #{match3}" unless output.include? match3
end
