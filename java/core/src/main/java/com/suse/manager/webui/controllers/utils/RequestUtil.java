...
public class RequestUtil {
    ...
    public static Optional<String> findStringParam(List<DiskFileItem> items, String name) {
        return findParamItem(items, name).map(item -> {
            try {
    ...
    /**
     * Find a form field parameter item by name
     * @param items the list of file items
     * @param name the field name to search for
     * @return optional of DiskFileItem if found
     */
    public static Optional<DiskFileItem> findParamItem(List<DiskFileItem> items, String name) {
        return items.stream()
