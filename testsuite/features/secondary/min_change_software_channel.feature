...
Feature: Assign child channel to a system
  ...
  Scenario: Assign a child channel to the system
    ...
    Then radio button "SLE-Product-SLES15-SP3-Pool for x86_64" is checked
    And I wait until I do not see "Loading..." text
    And I check "SLE15-SP3-Installer-Updates for x86_64"
    And I click on "Next"
    ...
    Then channel "SLE15-SP3-Installer-Updates for x86_64" should be enabled on "sle_minion"
  ...
  Scenario: Check the system is subscribed to the new channels
    ...
    Then radio button "SLE-Product-SLES15-SP3-Pool for x86_64" is checked
    And I wait until I do not see "Loading..." text
    And I should see "SLE15-SP3-Installer-Updates for x86_64" as checked

  Scenario: Check the new channels are enabled on the system
    When I refresh the metadata for "sle_minion"
    Then "16" channels should be enabled on "sle_minion"
    And channel "SLE-Product-SLES15-SP3-Pool for x86_64" should be enabled on "sle_minion"
    And channel "SLE15-SP3-Installer-Updates for x86_64" should be enabled on "sle_minion"
  ...
  Scenario: Cleanup: subscribe the system back to previous channels
    ...
    And I uncheck "SLE15-SP3-Installer-Updates for x86_64"
    And I click on "Next"
    Then I should see a "Confirm Software Channel Change" text
