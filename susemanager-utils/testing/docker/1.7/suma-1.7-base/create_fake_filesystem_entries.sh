...
mkdir -p /manager/spacewalk/admin/
touch /manager/spacewalk/admin/{spacewalk-service,rhn-config-schema.pl,rhn-config-satellite.pl}
...
mkdir -p /manager/spacewalk/setup/bin/
...
mkdir -p /manager/backend/rhn-conf
touch /manager/backend/rhn-conf/{rhn.conf,rhn_server.conf,rhn_server_satellite.conf}
