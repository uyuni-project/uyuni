...
PGSQL_DATA_SRC="/var/lib/pgsql/data"
PGSQL_DATA_DST="/var/lib/pgsql${NEW_VERSION}/data"

# Ensure the source directory actually exists
if [ ! -f "${PGSQL_DATA_SRC}/PG_VERSION" ]; then
  echo "Error: Cannot find PG_VERSION in ${PGSQL_DATA_SRC}. Check your source path."
  exit 1
fi
...
if [ "${OLD_VERSION}" -eq "${NEW_VERSION}" ]; then
  echo "Versions match (${OLD_VERSION}). No upgrade needed."
  shopt -s dotglob
  cp -a "${PGSQL_DATA_SRC}/." "${PGSQL_DATA_DST}/"
  chown postgres:postgres -R "${PGSQL_DATA_DST}"
  exit 0
fi
...
# Added --link for speed (uses hardlinks). Remove --link if you want a full copy (safer but slower).
