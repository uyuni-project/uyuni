...
Feature: Hub peripheral registration using access tokens
  ...
  @server3
  Scenario: Install the peripheral server on server3 with its own certificate (A-04)
    When I install the peripheral server on "server3" with its own self-signed certificate
