...
@scope_ansible
Feature: Operate an Ansible control node
...
@ssh_minion
  Scenario: Enable "Ansible control node" system type on SSH minion
...
@ssh_minion
  Scenario: Apply highstate and check that Ansible is installed on the SSH minion
    Given I am on the Systems overview page of this "ssh_minion"
    When I follow "States" in the content area
    And I click on "Apply Highstate"
    And I wait until event "Apply highstate scheduled by admin" is completed
