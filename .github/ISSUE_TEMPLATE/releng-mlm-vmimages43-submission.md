---
name: SUSE Multi-Linux Manager 4.3 VM Images submission
about: Use this template for submitting SUSE Multi-Linux Manager 4.3 VM Images
title: 'SUSE Multi-Linux Manager 4.3 VM Images X.Y.Z release'
...
- [ ] Report a new issue for autobuild team with the details of the needed projects at the [release-support board](https://gitlab.suse.de/buildops/release-support/-/issues/new).
  - [ ] autobuild team will have to create the release projects, like for example `SUSE:Products:SUSE-Manager-Server:4.3-2025-05:$arch`. 
  - [ ] check with them if projects are in place and if their release scripts are ready too. For the new projects to be created, ask autobuild to **NOT** use `QU` in the name because the projects with that keyword are not visible from the download page without login.
    As example, see:
    ~~~
    ...
    ~~~
- [ ] Ensure that all the `<version>` tags in the `.kiwi` files of both Server and Proxy images are set to the last released maintenance update version for SUSE Multi-Linux Manager (for example) `<version>4.3.11</version>`. If the version is older, prepare a SR for fixing it.
...
- [ ] Prepare the draft of the release announcement to be sent to the `multi-linux-manager@suse.de` mailing list.
...
- [ ] Check that the images are built in the submission project
  - [ ] https://build.suse.de/package/show/Devel:Galaxy:Manager:VMImages:4.3/SLES15-SP4-Manager-4-3-Server
  - [ ] https://build.suse.de/package/show/Devel:Galaxy:Manager:VMImages:4.3/SLES15-SP4-Manager-4-3-Proxy
- [ ] For the submission message specify the ijsc ID if any and the MU version. For example `Refreshed VM images for SUSE Multi-Linux Manager 4.3 based on SUSE Multi-Linux Manager 4.3.11. ijsc#MSQA-737`
  - [ ] Prepare the Server submission with `osc -A https://api.suse.de sr -m "Refreshed VM images for SUSE Multi-Linux Manager 4.3 based on SUSE Multi-Linux Manager 4.3.11. ijsc#MSQA-737" Devel:Galaxy:Manager:VMImages:4.3 SLES15-SP4-Manager-4-3-Server SUSE:SLE-15-SP4:Update:Products:Manager43:Update:CR`
  - [ ] Prepare the Proxy submission with `osc -A https://api.suse.de sr -m "Refreshed VM images for SUSE Multi-Linux Manager 4.3 based on SUSE Multi-Linux Manager 4.3.11. ijsc#MSQA-737" Devel:Galaxy:Manager:VMImages:4.3 SLES15-SP4-Manager-4-3-Proxy SUSE:SLE-15-SP4:Update:Products:Manager43:Update:CR`
- [ ] If autobuild already closed the previous gitlab issue. report a new issue for autobuild team at the [release-support board](https://gitlab.suse.de/buildops/release-support/-/issues/new).
  - [ ] specify the ID of the submit requests created against the CR project. They will have to accept your SRs. The maintenance team is not involved in this.
...
- [ ] When the sources finish to build and everything is published in the [CR project](https://build.suse.de/project/show/SUSE:SLE-15-SP4:Update:Products:Manager43:Update:CR), ask autobuild to release the images from [SUSE:SLE-15-SP4:Update:Products:Manager43:Update:CR](https://build.suse.de/project/show/SUSE:SLE-15-SP4:Update:Products:Manager43:Update:CR) to [SUSE:SLE-15-SP4:Update:Products:Manager43:Update:CR:ToTest](https://build.suse.de/project/show/SUSE:SLE-15-SP4:Update:Products:Manager43:Update:CR:ToTest)
- [ ] Ping `multi-linux-manager-qe` and tell them that the images are ready to be tested from the repo [SUSE:/SLE-15-SP4:/Update:/Products:/Manager43:/Update:/CR:/ToTest/images/](https://download.suse.de/ibs/SUSE:/SLE-15-SP4:/Update:/Products:/Manager43:/Update:/CR:/ToTest/images/)
