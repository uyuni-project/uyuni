...
{%- set missing = [] %}
{%- for k in required_mlm if not salt['pillar.get']('rke2_proxy:' ~ k) %}
  {%- do missing.append('rke2_proxy:' ~ k) %}
{%- endfor %}
{%- if not (salt['pillar.get']('rke2_proxy:proxy_fqdn') or salt['pillar.get']('primary_fqdn')) %}
  {%- do missing.append('rke2_proxy:proxy_fqdn (or SUMA primary_fqdn)') %}
{%- endif %}
{%- if not (salt['pillar.get']('rke2_proxy:server_fqdn') or salt['pillar.get']('mgr_server')) %}
  {%- do missing.append('rke2_proxy:server_fqdn (or SUMA mgr_server)') %}
{%- endif %}
