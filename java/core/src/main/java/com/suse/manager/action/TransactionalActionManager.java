...
public class TransactionalActionManager {
    ...
    private static List<MinionTransactionalActionHistory> findPendingRebootActions(
        ...
        String queryString = """
                ...
                """ + bootTime.map(time -> " AND history.prerequisiteAt < :bootTime").orElse("");
        var query = HibernateFactory.getSession().createQuery(queryString, MinionTransactionalActionHistory.class)
        ...
        bootTime.ifPresent(time -> query.setParameter("bootTime", new Date(time * 1000L)));
    ...
    public static TransactionalResult handleTransactionalResult(
        ...
        String prerequisiteResult = hasAfterRebootState ? formatResult(jsonResult) : null;
        if (failed) {
            history.recordTransactionalApplyFailed(prerequisiteResult);
            return TransactionalResult.failed("Failed to apply transactional states.");
        }
