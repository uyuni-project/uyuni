...
class BrokerHandler(SharedHandler):
    ...
    def handler(self):
        ...
        for _i in range(2):
            ...
            if error == "1003":  # invalid token
                ...
                log_debug(
                    2, "SUSE Manager Proxy Session Token expired, acquiring new one."
                )
                ...
                raise rhnFault(
                    1000,
                    _(
                        "SUSE Manager Proxy error (issues with proxy login). "
            ...
            raise rhnFault(
                1000,
                _(
                    "SUSE Manager Proxy error (unable to acquire proxy auth token). "
                    "Please contact your system administrator."
                ),
            )
    ...
    def __callLocalRepository(self, req_type, identifier, funct, params):
        ...
        if not f:
            raise rhnFault(
                1000,
                _("SUSE Manager Proxy configuration error: invalid function %s")
                % funct,
            )
