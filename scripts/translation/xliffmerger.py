...
def delete_trans_units(body_element, trans_units, units_to_remove):
    to_delete = [
        tr_unit for tr_unit in trans_units if tr_unit.attrib["id"] in units_to_remove
    ]
...
def add_trans_units(body_element, trans_units, units_to_add):
    to_add = [
        tr_unit for tr_unit in trans_units if tr_unit.attrib["id"] in units_to_add
    ]
    for item in to_add:
...
def process(original_file, translation_file):
    ...
    if len(trans_trans_units) == len(orig_trans_units):
        trans_units_srcs = {
            tr_unit.attrib["id"]: tr_unit.find("d:source", ns).text
            for tr_unit in trans_trans_units
        }
        org_trans_units_srcs = {
            tr_unit.attrib["id"]: tr_unit.find("d:source", ns).text
            for tr_unit in orig_trans_units
        }
        to_update = {
            k: org_trans_units_srcs[k]
