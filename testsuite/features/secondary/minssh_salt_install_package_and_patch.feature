...
Feature: Install a package and a patch on the SUSE SSH client via Salt through the UI
  ...
  Scenario: Pre-requisite: install virgo-dummy-1.0 package on SSH minion
    When I refresh the metadata for "sshminion"
    And I install old package "virgo-dummy-1.0" on this "sshminion" without error control
  ...
  Scenario: Pre-requisite: remove andromeda-dummy package from SSH minion
    ...
    And I refresh packages list via spacecmd on "sshminion"
  ...
  Scenario: Pre-requisite: refresh package list and check old packages installed on SSH minion
    ...
    And I wait until refresh package list on "sshminion" is finished
    Then spacecmd should show packages "virgo-dummy-1.0" installed on "sshminion"
    And I wait until package "andromeda-dummy-2.0-1.1" is removed from "sshminion" via spacecmd
  ...
  Scenario: Install a patch on the SSH minion
    ...
    When I wait for "virgo-dummy-2.0-1.1" to be installed on "sshminion"
    Then vendor change should be enabled for package actions on "sshminion"
  ...
  Scenario: Install a package on the SSH minion
    ...
    When I wait for "andromeda-dummy-2.0-1.1" to be installed on "sshminion"
    Then vendor change should be enabled for package actions on "sshminion"

  Scenario: Cleanup: disable test_repo_rpm_pool on SSH minion
    When I disable repository "test_repo_rpm_pool" on this "sshminion"

  Scenario: Cleanup: remove virgo-dummy package from SSH minion
    When I remove package "virgo-dummy" from this "sshminion" without error control
    And I refresh the metadata for "sshminion"
    And I refresh packages list via spacecmd on "sshminion"
    And I wait until refresh package list on "sshminion" is finished
