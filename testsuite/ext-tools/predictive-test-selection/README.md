...
- [Phase 1 - Gathering Training Data](#phase-1---gathering-training-data)
  ...
  - [6. Insert New Test Runs Into Database](#6-insert-new-test-runs-into-database)
  - [7. Generate Training Data Script](#7-generate-training-data-script)
...
## 6. Insert New Test Runs Into Database

### Functionality

The `insert-test-run-into-db` GitHub Action job, defined in [acceptance_tests_coordinator.yml](../../../.github/workflows/acceptance_tests_coordinator.yml), automatically executes with each new test run. The job performs the following operations:

1. Waits for PR tests completion.
2. Retrieves the modified files in the PR, GitHub run ID, PR number, and the commit SHA that triggered the test run.
3. Downloads all Cucumber JSON reports generated by the PR tests from GitHub Artifacts.
4. Constructs the Postgres database connection string using Uyuni GitHub secrets and stores it in an environment variable.
5. Triggers the [insert_test_run_into_db.py](../../../.github/scripts/insert_test_run_into_db.py) script with all collected data.

The [insert_test_run_into_db.py](../../../.github/scripts/insert_test_run_into_db.py) script performs the database insertion by reusing functionalities from the other scripts in this phase.

> [!WARNING]
> This module requires storing PR test Cucumber JSON reports as GitHub Artifacts. The `insert-test-run-into-db` GitHub job retrieves these artifacts after test execution completes. Since GitHub Artifacts are free for public repositories, we can continue storing them without concerns.

How this module currently fits in GitHub Actions

![insert_new_test_runs_into_db.png](images/insert_new_test_runs_into_db.png)
...
#### Why wasn't this functionality added directly to the workflow that runs the tests to access Cucumber reports locally without relying on GitHub Artifacts?

The PR acceptance tests are parallelized, which means adding database insertion directly after the test execution step would require inserting test results in multiple small pieces. This approach could lead to:
- Race conditions on the database
- Concurrency issues
- Inefficient database operations

The current approach waits for all tests to complete and all reports to be generated, then inserts them as a single batch. This method is simpler, less error-prone, more efficient, and has no drawbacks since GitHub Artifacts are free for public repositories.
...
If a test run contains a feature with a different scenario count than what's stored in the database, the database scenario count is updated to reflect the current count.

> [!NOTE]
> The current scenario counting method may not be entirely accurate. Some scenarios within features are always skipped due to tags that exclude them from GitHub Actions or non-SUSE Manager environments. These skipped scenarios should not be counted in the total scenario count for GitHub Acceptance tests. This can be addressed by modifying the scenario counting logic to exclude scenarios with these tags, but this improvement is currently low priority.

#### Why does the GitHub Action job only run in `uyuni-project/uyuni`?

The job is restricted to the original Uyuni repository (`uyuni-project/uyuni`) because forks don't have access to the Postgres database secrets. A condition prevents the job from executing on forks to avoid failures.

</details>

## 7. Generate Training Data Script
