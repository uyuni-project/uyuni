...
Feature: Add a Maintenance Update custom channel and the custom repositories for monitoring_server
  ...
  Scenario: Add the custom child channel for monitoring_server
    ...
    And I select the parent channel for the "sle15sp3_minion" from "Parent Channel"
  ...
  Scenario: Add the Maintenance update repositories for sle15sp3_minion
    When I create the MU repositories for "sle15sp3_minion"
  ...
  Scenario: Add the custom repositories to the custom channel for monitoring_server
    ...
    And I select the MU repositories for "sle15sp3_minion" from the list
