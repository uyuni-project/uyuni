...
@alma10_sshminion
Feature: Migrate a Alma 10 Salt SSH minion to SUSE Liberty Linux 10 using Liberate Formula

  Scenario: Prerequisite: Create Liberate System Group for SLL 10
    Given I am authorized for the "Admin" section
    When I follow the left menu "Systems > System Groups"
    And I follow "Create Group"
    And I enter "sll10-ssh-migration" as "name"
    And I enter "Systems to be converted to SUSE Liberty Linux 10" as "description"
    And I click on "Create Group"
    Then I should see a "System group sll10-ssh-migration created." text
  ...
  Scenario: Prerequisite: Prepare activation key for SLL 10
    ...
    And I enter "sll10_ssh_migration_key" as "key"
    And I enter "sll10_ssh_migration_key" as "description"
    ...
    Then I wait until I see "Activation key sll10_ssh_migration_key has been created." text
    ...
    Then I wait until I see "Activation key sll10_ssh_migration_key has been modified." text
    When I follow "Groups" in the content area
    And I follow first "Join"
    And I check the row with the "sll10-ssh-migration" link
  ...
  Scenario: Prerequisite: Set environment variables for the bootstrap script on the Alma 10 minion
    When I set the "ACTIVATION_KEYS" environment variable to "1-sll10_ssh_migration_key" on this "alma10_sshminion"
    And I set a reactivation key as environment variable on this "alma10_sshminion"
    And I navigate to the Systems overview page of this "alma10_sshminion"
    And I follow "Details" in the content area
    And I follow "Reactivation" in the content area
    Then I should see a "Key:" text

  Scenario: Migrate the Alma 10 SSH minion to SSL 10 with a bootstrap script
    ...
    And I execute the "bootstrap.sh" bootstrap script on this "alma10_sshminion"
    When I delete "alma10_sshminion" key in the Salt master
    And I follow the left menu "Salt > Keys"
    And I should see a "Keys" text in the content area
    And I wait at most 60 seconds until Salt master sees "alma10_sshminion" as "unaccepted"
    And I accept "alma10_sshminion" key
    And I wait at most 10 seconds until Salt master sees "alma10_sshminion" as "accepted"

  Scenario: Check the migration is successful for this minion
    Given I am on the Systems overview page of this "alma10_sshminion"
    ...
    Then file "/etc/sysconfig/liberated" should contain "LIBERATED_FROM=" on "alma10_sshminion"
