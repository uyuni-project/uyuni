...
The Root CA certificate of `proxy-cert` is expected in a ConfigMap `uyuni-ca` with the certificate in the `ca.crt` key. The CA cert is embedded in `config.yaml` (generated by the server) under the `ca_crt` field. Like `server_crt`, it contains an OpenSSL text dump followed by the PEM block — extract only the PEM block:
...
# 2. Extract server.crt (PEM block only) and server.key from httpd.yaml under httpd.server_crt/server_key.
#    Extract ca.crt (PEM block only) from config.yaml under ca_crt.
#    Both fields contain an OpenSSL text dump before the PEM block — keep only the PEM block.
...
# 3. Prepare the new namespace, secret and configmap in advance
kubectl create namespace uyuni-proxy
kubectl -n uyuni-proxy create secret tls proxy-cert \
  --cert=/root/proxy-config/server.crt --key=/root/proxy-config/server.key
kubectl -n uyuni-proxy create configmap uyuni-ca \
  --from-file=ca.crt=/root/proxy-config/ca.crt
```
