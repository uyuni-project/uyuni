...
@peripheral2
Feature: Hub setup and peripheral registration via administrator credentials for peripheral2
  ...
  Scenario: Generate hub-signed SSL certificates for the peripheral
    When I generate hub-signed SSL certificates for "peripheral2" on "hub"

  Scenario: Transfer the hub-signed SSL certificates to the peripheral
    When I copy the hub-signed SSL certificates for "peripheral2" from "hub"
    And I trust the hub CA certificate on "peripheral2"

  Scenario: Install the peripheral server with hub-signed certificates
    When I install the peripheral server on "peripheral2" using the hub-signed certificates
    And I wait until the server on "peripheral2" is ready

  Scenario: Apply testsuite configuration to the peripheral server
    When I apply the testsuite configuration on the peripheral server "peripheral2"
    And I wait until the server on "peripheral2" is ready
  ...
  Scenario: Register peripheral2 as a peripheral with administrator credentials
    When I add "peripheral2" as peripheral using administrator credentials
  ...
  Scenario: Verify peripheral appears in system list as a Foreign system
    ...
    Then I should see "peripheral2" in the system list with "Foreign" system type
  ...
  Scenario: Verify access token was created on hub after registration
    ...
    And I should see the name of "peripheral2"
  ...
  Scenario: Verify Hub Details are populated on the peripheral
    ...
    Then the Hub Details page on "peripheral2" should show the hub FQDN
