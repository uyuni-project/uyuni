        ...
        try:
            ...
            if statinfo.st_size == 0:
                raise_with_tb(
                    rhnFault(
                        1000,
                        _(
                            "SUSE Manager Proxy is not configured, systemid file is empty. "
            ...
            raise_with_tb(
                rhnFault(
                    1000,
                    _(
                        "SUSE Manager Proxy is not configured, systemid file is missing. "
                        "Please contact your system administrator."
                    ),
                ),
                sys.exc_info()[2],
            ...
            log_error(
                "unable to stat %s: %s" % (ProxyAuth.__systemid_filename, repr(e))
            )
            raise_with_tb(
                rhnFault(
                    1000,
                    _(
                        "SUSE Manager Proxy error (SUSE Manager Proxy systemid has wrong permissions?). "
                        "Please contact your system administrator."
                    ),
                ),
                sys.exc_info()[2],
...
"""
            ...
            raise_with_tb(
                rhnFault(
                    1000,
                    _(
                        "SUSE Manager Proxy error (auth caching issue). "
                        "Please contact your system administrator."
                    ),
                ),
                sys.exc_info()[2],
    ...
    def login(self):
        ...
        for _i in range(self.__nRetries):
            try:
                ...
                Traceback(
                    "ProxyAuth.login (Fault) - SUSE Manager Proxy not "
                    "able to log in."
                )
                ...
                raise_with_tb(
                    rhnFault(
                        1000,
                        _(
                            "SUSE Manager Proxy error (during proxy login). "
                            "Please contact your system administrator."
                        ),
                    ),
                    sys.exc_info()[2],
            ...
            if error:
                if error[0] in ("xmlrpclib.ProtocolError", "socket.error", "socket"):
                    raise rhnFault(
                        1000,
                        _(
                            "SUSE Manager Proxy error (error: %s). "
                            "Please contact your system administrator."
                        )
                        % error[0],
                    )
                if error[0] in ("rhn.SSL.SSL.SSLError", "socket.sslerror"):
                    raise rhnFault(
                        1000,
                        _(
                            "SUSE Manager Proxy error (SSL issues? Error: %s). "
                            "Please contact your system administrator."
                        )
                        % error[0],
                    )
    ...
    def __getXmlrpcServer():
        ...
        if CFG.CA_CHAIN:
            if not os.access(CFG.CA_CHAIN, os.R_OK):
                log_error(
                    "ERROR: missing or cannot access (for ca_chain): %s" % CFG.CA_CHAIN
                )
                raise rhnFault(
                    1000,
                    _(
                        "SUSE Manager Proxy error (file access issues). "
                        "Please contact your system administrator. "
