...
if __name__ == "__main__":
    ...
    parser.add_argument("-u", "--user", default=os.getenv("USER"), help="The SSH username. Defaults to current user if SSH config is not used.")
    parser.add_argument("-p", "--password", help="The SSH password (used if key-based auth fails).")
    parser.add_argument("-k", "--key", default=os.path.expanduser("~/.ssh/id_rsa"), help="Path to the SSH private key (defaults to ~/.ssh/id_rsa).")
    parser.add_argument("--use-root-pass", action="store_true", help="Use 'root' as username and 'linux' as password for connection.")

    args = parser.parse_args()

    if args.use_root_pass:
        args.user = "root"
        args.password = "linux"
        args.key = None
