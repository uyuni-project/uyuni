...
check_bootstrap_dbg:
  cmd.run:
    - name: {{ utils.find_binary('echo') }} "{{ salt_minion_installed }}"
...
{%- set pending_transaction_id = salt['cmd.run'](utils.find_binary('snapper') ~ ' --no-dbus list --columns=number | ' ~ utils.find_binary('grep') ~ ' "+" | tr -d "+"', python_shell=True) %}
...
{{ salt_minion_name }}:
  mgrcompat.module_run:
    ...
    - command: {{ utils.find_binary('systemctl') }} enable {{ salt_minion_name }}
    - snapshot: continue
    - require:
      - salt-minion-package
      - host: mgr_server_localhost_alias_absent
...
copy_transactional_conf_file_to_etc:
  file.copy:
    ...
    - unless:
      - {{ utils.find_binary('test') }} -f /etc/transactional-update.conf
...
transactional_update_set_reboot_method_systemd:
  file.keyvalue:
    ...
    - unless:
      - {{ utils.find_binary('grep') }} -P '^(?=[\s]*+[^#])[^#]*(REBOOT_METHOD=(?!auto))' /etc/transactional-update.conf

disable_reboot_timer_transactional_minions:
  cmd.run:
    - name: {{ utils.find_binary('systemctl') }} disable transactional-update.timer
{%- endif %}
