...
@server3
Feature: Hub setup and peripheral registration via administrator credentials for server3
  ...
  Scenario: Generate hub-signed SSL certificates for the peripheral
    When I generate hub-signed SSL certificates for "server3" on "server"

  Scenario: Transfer the hub-signed SSL certificates to the peripheral
    When I copy the hub-signed SSL certificates for "server3" from "server"
    And I trust the hub CA certificate on "server3"

  Scenario: Install the peripheral server with hub-signed certificates
    When I install the peripheral server on "server3" using the hub-signed certificates
    And I wait until the server on "server3" is ready

  Scenario: Apply testsuite configuration to the peripheral server
    When I apply the testsuite configuration on the peripheral server "server3"
    And I wait until the server on "server3" is ready
  ...
  Scenario: Register server3 as a peripheral with administrator credentials
    When I add "server3" as peripheral using administrator credentials
  ...
  Scenario: Verify peripheral appears in system list as a Foreign system
    ...
    Then I should see "server3" in the system list with "Foreign" system type
  ...
  Scenario: Verify access token was created on hub after registration
    ...
    And I should see the name of "server3"
  ...
  Scenario: Verify Hub Details are populated on the peripheral
    ...
    Then the Hub Details page on "server3" should show the hub FQDN
