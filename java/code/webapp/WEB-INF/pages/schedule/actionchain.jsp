                ...
                <%@ taglib uri="http://rhn.redhat.com/tags/list" prefix="rl" %>
                    ...
                    <body>
                        <script type="text/javascript"
                            src="/javascript/jquery-ui.js?cb=${rhn:getConfig('web.buildtimestamp')}"></script>
                        <script type="text/javascript"
                            src="/javascript/actionchain.js?cb=${rhn:getConfig('web.buildtimestamp')}"></script>
                        <rhn:toolbar base="h1" icon="header-chain"
                            ...
                            <a id="label-link" href="#">
                                <span id="label-link-text">
                                    <c:out value="${actionChain.label}" />
                                </span>
                                <i class="fa fa-pencil"></i>
                            </a>
                            <input id="label-input" type="text" value='<c:out value="${actionChain.label}"/>'
                                maxlength="256" autocomplete="off" hidden />
                        </rhn:toolbar>
                        ...
                        <div class="spacewalk-toolbar-h1">
                            <div class="spacewalk-toolbar">
                                <a class="btn btn-danger" data-bs-toggle="modal" href="#confirm-modal"><i
                        ...
                        <div class="panel panel-default">
                            <div class="panel-heading">
                                <h4>
                                    <bean:message key="actionchain.jsp.edit" />
                                </h4>
                            </div>
                                    ...
                                    <div class="table-responsive">
                                        <table class="table table-striped">
                                            <thead>
                                                <tr>
                                                    <th>
                                                        <bean:message key="actionchain.jsp.action" />
                                                    </th>
                                                    <th>
                                            ...
                                            <tbody class="action-chain" data-action-chain-id="${param.id}"
                                                ...
                                                <c:forEach items="${groups}" var="group">
                                                    <tr class="group" data-sort-order="${group.sortOrder}">
                                                        <td>
                                                            <a class="system-list-show-hide" href="#"><i
                                                                    class="fa fa-plus-square"></i></a>

                                                            <span class="counter">${group.sortOrder + 1}</span>.

                                                            <bean:message key="actionchain.jsp.${group.actionTypeLabel}"
                                                                arg0="${group.relatedObjectDescription}" />

                                                            <strong class="system-counter">
                                                                ${group.systemCount}
                                                            </strong>

                                                            <span class="singular-label" <c:if
                                                                test="${group.systemCount != 1}">hidden</c:if>>
                                                                <bean:message key="actionchain.jsp.system" />
                                                            </span>
                                                            <span class="plural-label" <c:if
                                                                test="${group.systemCount == 1}">hidden</c:if>>
                                                                <bean:message key="actionchain.jsp.systems" />
                                                            </span>

                                                            <div class="system-list" id="system-list-${group.sortOrder}"
                                                                hidden></div>
                                                        </td>
                                                        <td>
                                                            <a class="delete-group" href="#"><i
                                                                    class="fa fa-trash-o"></i>delete action</a>
                                                        </td>
                                                    </tr>
                                                </c:forEach>
                                            </tbody>
                                ...
                                <div id="action-chain-save-input" class="form-group" hidden>
                                    <div class="col-md-offset-3 offset-md-3 col-md-6">
                                        <button class="btn btn-primary" id="save">
                                            <bean:message key="actionchain.jsp.save" />
                                        </button>
                                        <button class="btn btn-default" id="cancel">
                                            <bean:message key="actionchain.jsp.cancel" />
                                        </button>
                                    </div>
                                </div>
                            </div>
                        ...
                        <div class="panel panel-default">
                            <div class="panel-heading">
                                <h4>
                                    <bean:message key="actionchain.jsp.schedule" />
                                </h4>
                            </div>
                            <div class="panel-body">
                                <form action="/rhn/schedule/ActionChain.do?id=${param.id}" method="post"
                                    ...
                                    <div class="form-horizontal">
                                        <html:hidden property="dispatch"
                                            value="${rhn:localize('actionchain.jsp.saveandschedule')}" />
                                        <rhn:csrf />
                                        <rhn:submitted />

                                        <c:choose>
                                            <%-- When there are multiple schedules, we do not display date picker nor
                                                the maint. window picker --%>
                                                <c:when test="${maintenanceWindowsMultiSchedules}">
                                                    <div class="alert alert-info">
                                                        <bean:message
                                                            key="schedule.jsp.multiple_maintenance_schedules" />
                                                    </div>
                                                </c:when>
                                                <%-- When maintenance windows are set but empty, we do not display date
                                                    picker nor the maint. window picker --%>
                                                    <c:when
                                                        test="${maintenanceWindows != null && empty maintenanceWindows}">
                                                        <div class="alert alert-info">
                                                    ...
                                                    <c:otherwise>
                                                        <div class="form-group">
                                                            <div class="col-sm-12">
                                                                <p>
                                                                    <bean:message
                                                                        key="actionchain.jsp.schedulesummary" />
                                                                </p>
                                                            </div>
                                                        </div>
                                                        <c:choose>
                                                            <%-- When there are no maintenance windows, display the
                                                                usual date picker --%>
                                                                <c:when test="${maintenanceWindows == null}">
                                                                    <rhn:hidden name="schedule_type" value="date" />
                                                                    <div class="form-group">
                                                                        <div
                                                                            class="col-md-offset-3 offset-md-3 col-md-6">
                                                                            <jsp:include
                                                                ...
                                                                <c:otherwise>
                                                                    <rhn:hidden name="schedule_type"
                                                                        value="maintenance_window" />
                                                                    <div class="col-md-3">
                                                                        <bean:message key="schedule.jsp.maint-window" />
                                                                    </div>
                                                                    <jsp:include
                                                                        page="/WEB-INF/pages/common/fragments/maintenance-window-picker.jsp" />
                                                                </c:otherwise>
                                                        </c:choose>
                                                        <div class="form-group">
                                                            <div class="col-md-offset-3 offset-md-3 col-md-6">
                                                                <button type="button" class="btn btn-primary"
                                                                    id="save-and-schedule">
                                                                    <bean:message
                                                                        key="actionchain.jsp.saveandschedule" />
                                                                </button>
                                                            </div>
                                                        </div>
                                                    </c:otherwise>
                        ...
                        <%-- Modal delete confirm dialog --%>
                            ...
                            <%-- Stay or leave page dialog text. Not shown by some browsers. --%>
                                <span id="before-unload" hidden>
                                    <bean:message key="actionchain.jsp.stayorleave" />
                                </span>
                    </body>
