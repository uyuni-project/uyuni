...
FileOrPath = typing.Union[Path, typing.IO]
...
def verify_file(
    *,
    signed_file: FileOrPath,
    detached_signature_file: typing.Optional[FileOrPath] = None,
    ...
    Args:
      signed_file: The path to, or file object of, a file that is signed.
      detached_signature_file: The path to, or file object of, a detached signature, optional.
    ...
    if detached_signature_file:
        ...
        detached_signature_fname = None
        logger.debug(
            "No detached signature_file, assuming '%s' contains the signature.",
            signed_fname,
        )

    for f in (signed_fname, detached_signature_fname, keyring):
        if f and not os.access(path=f, mode=os.R_OK):
            raise OSError(f"Can't access {f}.")

    completed = _run(
        signed_file=signed_fname,
        signature_file=detached_signature_fname,
        keyring=keyring,
    )
