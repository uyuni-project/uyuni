...
for schema in "${available_schemas[@]}"; do
    ...
    schema_version=$(sudo -i podman exec server rpm -q --queryformat '%{version}' "${schema_name}")
...
sudo -i podman exec server bash -c "rsync -v /testsuite/dockerfiles/server-all-in-one-dev/mirror/*.json /mirror"
