...
const WebCalendar = (props: WebCalendarProps) => {
  ...
  useEffect(() => {
    ...
    jQuery(".fc-backButton-button").attr("data-testid", "back").html('<div class="fa fa-angle-left"></div>');
    jQuery(".fc-nextButton-button").attr("data-testid", "next").html('<div class="fa fa-angle-right"></div>');
    jQuery(".fc-skipBackButton-button")
      .html('<div class="fa fa-angle-double-left"></div>')
      .attr("data-testid", "skip-back")
      .prop("title", t("Skip to the last maintenance window"));
    jQuery(".fc-skipNextButton-button")
      .html('<div class="fa fa-angle-double-right"></div>')
      .attr("data-testid", "skip-next")
  ...
  const getEvents = (operation) => {
    ...
    if (operation === "initial" || needsUpdate(date)) {
      ...
      const endpoint = `/rhn/manager/api/maintenance/events/${operation}/${
        props.type
      }/${startOfWeek}/${date.valueOf()}/${props.id}`;
  ...
  const onSkipBack = () => {
    ...
    const startOfWeek = getApi().currentDataManager.data.dateEnv.weekDow;
    const endpoint = `/rhn/manager/api/maintenance/events/skipBack/${props.type}/${startOfWeek}/${date.valueOf()}/${
      props.id
    }`;
    return Network.get(endpoint)
      .then((events) => {
        if (events.length === 0) {
          ...
          const filteredEvents = events.filter(
            (event) => moment.parseZone(event.start).month() !== moment(currentDate).month()
          );
  ...
  const onSkipNext = () => {
    ...
    const startOfWeek = getApi().currentDataManager.data.dateEnv.weekDow;
    const endpoint = `/rhn/manager/api/maintenance/events/skipNext/${props.type}/${startOfWeek}/${date.valueOf()}/${
      props.id
    }`;
    return Network.get(endpoint)
      .then((events) => {
        if (events.length === 0) {
          ...
          setEvents(processEvents(events));
          // Skip to next event that is not in current month
          const firstEvent = moment.parseZone(
            events.filter((event) => moment.parseZone(event.start).month() !== moment(currentDate).month())[0].start
          );
  ...
  const needsUpdate = (date) => {
    return date.month() !== moment(currentDate).month() || date.year() !== moment(currentDate).year();
  };
    ...
    if (props.eventNames && props.eventNames.length > 0) {
      props.eventNames.forEach((name, i) =>
        events
          ...
          .map((event) => Object.assign(event, { color: colors[(i + colors.length) % colors.length] }))
      );
    }
  ...
  const setEventContent = (eventContent) => {
    if (eventContent.view.type === "dayGridMonth") {
      return (
        <>
          {eventContent.timeText} {eventContent.event.extendedProps.startTimezone} {eventContent.event.title}
        </>
      );
    } else {
      ...
      return (
        <>
          {timeText[0] +
            " " +
            eventContent.event.extendedProps.startTimezone +
            " - " +
            timeText[1] +
            " " +
            eventContent.event.extendedProps.endTimezone}{" "}
