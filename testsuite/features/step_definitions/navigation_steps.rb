...
Then(/^I should see a "(.*)" text in the content area$/) do |text|
  within('#spacewalk-content') do
    raise "Text '#{text}' not found" unless check_text_and_catch_message_request_timeout?(text)
  end
end
...
When(/^I wait until I see "([^"]*)" text$/) do |text|
  raise "Text '#{text}' not found" unless check_text_and_catch_message_request_timeout?(text, timeout: DEFAULT_TIMEOUT)
end
...
When(/^I wait at most (\d+) seconds until I see "([^"]*)" text$/) do |seconds, text|
  raise "Text '#{text}' not found" unless check_text_and_catch_message_request_timeout?(text, timeout: seconds.to_i)
end

When(/^I wait until I see "([^"]*)" text or "([^"]*)" text$/) do |text1, text2|
  raise "Text '#{text1}' or '#{text2}' not found" unless check_text_and_catch_message_request_timeout?(text1, text2: text2, timeout: DEFAULT_TIMEOUT)
end
...
Given(/^I access the host the first time$/) do
  visit Capybara.app_host
  raise "Text 'Create #{product} Administrator' not found" unless check_text_and_catch_message_request_timeout?("Create #{product} Administrator")
end
...
Then(/^I wait until table row for "([^"]*)" contains "([^"]*)"$/) do |arg1, arg2|
  ...
  within(:xpath, xpath_query) do
    raise "xpath: #{xpath_query} has no content #{arg2}" unless check_text_and_catch_message_request_timeout?(arg2, timeout: DEFAULT_TIMEOUT)
  end
end
...
Then(/^I should see "([^"]*)" systems selected for SSM$/) do |arg|
  within(:xpath, '//span[@id="spacewalk-set-system_list-counter"]') do
    raise "There are not #{arg} systems selected" unless check_text_and_catch_message_request_timeout?(arg)
  end
end
...
Then(/^I should see a "([^"]*)" text or "([^"]*)" text$/) do |text1, text2|
  raise "Text '#{text1}' and '#{text2}' not found" unless check_text_and_catch_message_request_timeout?(text1, text2: text2)
end

Then(/^I should see "([^"]*)" short hostname$/) do |host|
  system_name = get_system_name(host).partition('.').first
  raise "Hostname #{system_name} is not present" unless check_text_and_catch_message_request_timeout?(system_name)
end
...
Then(/^I should not see "([^"]*)" hostname$/) do |host|
  system_name = get_system_name(host)
  raise "Hostname #{system_name} is present" if check_text_and_catch_message_request_timeout?(system_name)
end
...
Then(/^I should see a "([^"]*)" text in element "([^"]*)"$/) do |text, element|
  within(:xpath, "//div[@id=\"#{element}\" or @class=\"#{element}\"]") do
    raise "Text '#{text}' not found in #{element}" unless check_text_and_catch_message_request_timeout?(text)
  end
end

Then(/^I should not see a "([^"]*)" text in element "([^"]*)"$/) do |text, element|
  within(:xpath, "//div[@id=\"#{element}\" or @class=\"#{element}\"]") do
    raise "Text '#{text}' found in #{element}" if check_text_and_catch_message_request_timeout?(text)
  end
end

Then(/^I should see a "([^"]*)" or "([^"]*)" text in element "([^"]*)"$/) do |text1, text2, element|
  within(:xpath, "//div[@id=\"#{element}\" or @class=\"#{element}\"]") do
    raise "Texts #{text1} and #{text2} not found in #{element}" unless check_text_and_catch_message_request_timeout?(text1, text2: text2)
  end
end
...
Then(/^I click on the filter button until page does not contain "([^"]*)" text$/) do |text|
  repeat_until_timeout(message: "'#{text}' still found") do
    break unless check_text_and_catch_message_request_timeout?(text)
...
Then(/^I click on the filter button until page does contain "([^"]*)" text$/) do |text|
  repeat_until_timeout(message: "'#{text}' was not found") do
    break if check_text_and_catch_message_request_timeout?(text)
    find('button.spacewalk-button-filter').click
    has_text?('is filtered', wait: 10)
  end
end
...
Then(/^I should see "([^"]*)" hostname as first search result$/) do |host|
  ...
  within(:xpath, '//section') do
    row = find(:xpath, "//div[@class='table-responsive']/table/tbody/tr[.//td]", match: :first)
    within(row) do
      raise "Text '#{system_name}' not found" unless check_text_and_catch_message_request_timeout?(system_name)
    end
  end
end
