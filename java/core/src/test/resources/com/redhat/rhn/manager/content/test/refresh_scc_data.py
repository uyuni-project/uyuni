...
def parse_arguments():
    ...
    parser.add_argument(
        "-u", "--username", dest="username", help="SCC username", required=True
    )
    parser.add_argument(
        "-p", "--password", dest="password", help="SCC password", required=True
    )
    parser.add_argument(
        "-r",
        "--real-tokens",
...
products = get_paginated(
    connection, headers, "/connect/organizations/products/unscoped"
)
...
if args.real_tokens:
    ...
    for repo_num in range(0, len(repositories)):
        repositories[repo_num]["url"] = repo_url_regex.sub(
            r"\1?my-fake-token", repositories[repo_num]["url"]
        )
...
if args.subscriptions:
    ...
    print("         This could leak private data!")
    subscriptions = get_paginated(
        connection, headers, "/connect/organizations/subscriptions"
    )
