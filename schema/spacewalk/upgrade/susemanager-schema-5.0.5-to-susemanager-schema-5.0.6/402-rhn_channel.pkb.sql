...
update pg_settings set setting = 'rhn_channel,' || setting where name = 'search_path';
   ...
   if deleting_server = 0 then
    ...
    begin
        ...
        delete from suseChannelModuleNewestPackage
              where channel_id = channel_id_in
                and (package_name_id_in is null
                     or name_id = package_name_id_in);
        ...
        INSERT INTO suseChannelModuleNewestPackage
          (channel_id, name_id, evr_id, package_id, package_arch_id, appstream_id)
          SELECT channel_id, name_id, evr_id, package_id, package_arch_id, appstream_id
          FROM suseChannelModuleNewestPackageView
          WHERE channel_id = channel_id_in
          AND (package_name_id_in IS NULL OR name_id = package_name_id_in);
