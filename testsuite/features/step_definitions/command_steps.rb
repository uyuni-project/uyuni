...
#
# The non-k3s path needs a local CA under /root/ssl-build to sign the proxy's cert. In a
# hub/peripheral topology the peripheral shares the hub's CA but doesn't hold a copy of it (see
# hub_test_plan.md) - and proxy_container_config_generate_cert validates its "managing server"
# argument against the server it runs on, so it can't be run on the hub on the peripheral's
# behalf either. So when the target server isn't 'server', copy the hub's CA files over first.
#
# Usage:
...
When(/^I generate the configuration "([^"]*)" of containerized (proxy|proxy2|proxy3) on (?:the )?(server|server2|server3)$/) do |file_path, proxy_target, host|
  if running_k3s?
    ...
    unless host == 'server'
      get_target(host).run('mkdir -p /root/ssl-build', check_errors: false)
      %w[RHN-ORG-TRUSTED-SSL-CERT RHN-ORG-PRIVATE-SSL-KEY].each do |ca_file|
        get_target('server').extract("/root/ssl-build/#{ca_file}", "/tmp/#{ca_file}")
        get_target(host).inject("/tmp/#{ca_file}", "/root/ssl-build/#{ca_file}")
      end
    end
...
When(/^I (upgrade|install) "([^"]*)" on "([^"]*)" using the API(?: from (server|server2|server3))?$/) do |action, package, host, mgr_server|
  ...
  unless action_triggered
    log "No #{action} action needed for \"#{package}\" on \"#{host}\": already up to date."
    next
  end
