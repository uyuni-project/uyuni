...
Feature: Prepare buildhost and build OS image for SLES 15 SP3
  ...
  Scenario: Prepare activation key for SLES 15 SP3 build host
  ...
  Scenario: Bootstrap the SLES 15 SP3 build host
    ...
    And I wait at most 10 seconds until Salt master sees "sle15sp3_buildhost" as "unaccepted"
    And I accept "sle15sp3_buildhost" key in the Salt master
    And I wait until onboarding is completed for "sle15sp3_buildhost"

  Scenario: Turn the SLES 15 SP3 build host into an OS image build host
  ...
  Scenario: Apply the highstate to the SLES 15 SP3 build host
    Given I am on the Systems overview page of this "sle15sp3_buildhost"
    When I wait until no Salt job is running on "sle15sp3_buildhost"
    And I apply highstate on "sle15sp3_buildhost"
    And I wait until file "/var/lib/Kiwi/repo/rhn-org-trusted-ssl-cert-osimage-1.0-1.noarch.rpm" exists on "sle15sp3_buildhost"

  Scenario: Create an OS image profile for SLES 15 SP3 with activation key
  ...
  Scenario: Build Kiwi image for SLES 15 SP3
    When I follow the left menu "Images > Build"
    And I select "suse_os_image_15" from "profileId"
    And I select the hostname of "sle15sp3_buildhost" from "buildHostId"
    And I click on "submit-btn"

  Scenario: Check the built SLES 15 SP3 OS image
    Given I am on the Systems overview page of this "sle15sp3_buildhost"
  ...
  Scenario: Move the SLES 15 SP3 image to the branch server
    When I apply state "image-sync" to "proxy"
    Then the image for "sle15sp3_terminal" should exist on the branch server
