...
When(/^I stop salt-minion on "(.*?)"$/) do |minion|
  ...
  node.run("systemctl stop #{pkgname}", check_errors: false) if %w[
    ceos_minion
    ubuntu_minion
    kvm_server
    xen_server
...
When(/^I start salt-minion on "(.*?)"$/) do |minion|
  ...
  node.run("systemctl restart #{pkgname}", check_errors: false) if %w[
    ceos_minion
    ubuntu_minion
    kvm_server
    xen_server
...
When(/^I uninstall Salt packages from "(.*?)"$/) do |host|
  ...
  pkgs = $product == 'Uyuni' ? 'env-salt-minion' : 'salt salt-minion'
...
When(/^I perform a full salt minion cleanup on "([^"]*)"$/) do |host|
  ...
  if host.include? 'ceos'
    ...
    pkgs = 'salt-common salt-minion' if $product != 'Uyuni'
    node.run(
      "apt-get --assume-yes remove #{pkgs} && apt-get --assume-yes purge #{pkgs} && apt-get --assume-yes autoremove",
      check_errors: false
    )
  else
