...
ca_crt: |
$(sed 's/^/  /' /root/RHN-ORG-TRUSTED-SSL-CERT)
...
httpd:
  ...
  server_crt: |
$(sed 's/^/      /' /root/server.crt)
  server_key: |
$(sed 's/^/      /' /root/server.key)
EOF

  ssh-keygen -t rsa -b 4096 -C "salt@uyuni-server-all-in-one-test" -f /root/id_openssh_rsa -N ""
  cat <<EOF > ssh.yaml
ssh:
  server_ssh_key_pub: |
  ...
  server_ssh_push: |
$(sed 's/^/    /' /root/id_openssh_rsa)
  server_ssh_push_pub: |
$(sed 's/^/    /' /root/id_openssh_rsa.pub)
EOF
}
