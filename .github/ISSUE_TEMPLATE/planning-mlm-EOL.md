---
name: SUSE Multi-Linux Manager / Client tools EOL
about: Use this template when a product goes EOL in SUSE Multi-Linux Manager (the product itself or client tools)
title: 'Epic: XXX EOL'
labels: ["vega-squad", "qe-squad", "docs-squad", "orion-squad", "naica-squad", "epics"]
assignees: ''
projects: ["SUSE/37"]
---
...
- [ ] Dev: Change repositories if the product move them to an archive place (Ubuntu, Debian, Rocky and Alma sometimes do it)
...
- [ ] Releng: For client tools, remove the product the CI installation checks (`manager-*-releng-SUSE_Manager_Client_Tools_Installation_Check`)
  ...
  - [ ] Manager-X.Y
- [ ] Releng: For client tools, remove the product from the CI build checks (`*-dev-at-obs`)
  ...
  - [ ] Manager-X.Y
- [ ] Releng: For Server/Proxy, remove the [Jenkins jobs and views](https://ci.suse.de/view/Manager/) for the relevant version
