...
if [ -f "${SRPM_PKG_DIR}/Chart.yaml" ]; then
    ...
    if [ "${OSCAPI}" == "https://api.suse.de" ]; then
        ...
        NAMESPACE="suse\/manager\/${VERSION}\/"

        sed "s/^\(#\!BuildTag: \)uyuni\(.\+\)\$/\1${NAMESPACE}x86_64\2\n\1${NAMESPACE}aarch64\2\n\1${NAMESPACE}s390x\2\n\1${NAMESPACE}ppc64le\2/" -i ${SRPM_PKG_DIR}/Chart.yaml
        sed "/^#\!BuildTag:/s/\(${NAMESPACE}\)\([^/]\+\)\(\/[^:]\+\):latest\$/\1\2\3:latest \1\2\3:${SEMANTIC_VERSION} \1\2\3:${SEMANTIC_VERSION}.%RELEASE%/g" -i ${SRPM_PKG_DIR}/Chart.yaml
        ...
        NAME="uyuni\/${NAME}"
        sed "/^#\!BuildTag:/ s/$/ ${NAME}:${SEMANTIC_VERSION} ${NAME}:${SEMANTIC_VERSION}.%RELEASE%/" -i ${SRPM_PKG_DIR}/Chart.yaml
    fi
