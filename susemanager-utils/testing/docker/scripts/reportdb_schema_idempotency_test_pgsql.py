...
def get_all_files_from_pr(pr_file, schema_path):
    ...
    with open(pr_file, "r") as json_file:  #  pylint: disable=unspecified-encoding
        ...
        for file in files:
            ...
            if not re.search(
                r"^schema\/reportdb\/upgrade\/uyuni-reportdb-schema-\d+\.\d+[\d|.]*-to-uyuni-reportdb-schema-\d+\.\d+[\d|.]*\/[^\/]+$",  #  pylint: disable=line-too-long
                filename,
            ):
                continue
...
def get_all_files_since(version, schema_path):
    ...
    for element in sorted(os.listdir(schema_path)):
        element_ver = re.search(
            r"^uyuni-reportdb-schema-(\d+\.\d+[\d|.]*)-to-uyuni-reportdb-schema-\d+\.\d+[\d|.]*$",  #  pylint: disable=line-too-long
            element,
        )
        if not element_ver:
            continue
...
def get_all_files(start, end, schema_path):
    start_schema = "uyuni-reportdb-schema-%s" % start  #  pylint: disable=consider-using-f-string
    end_schema = "uyuni-reportdb-schema-%s" % end  #  pylint: disable=consider-using-f-string
...
def create_fake_migration_path(schema_path, new_version, pr_file=None, version=None):
    ...
    fake_path = (
        schema_path
        + "/uyuni-reportdb-schema-%s-to-uyuni-reportdb-schema-%s/"  #  pylint: disable=consider-using-f-string
    ...
    for migration_file in files:
        ...
        fakedir = pcom.pop()
        fakedir = "uyuni-reportdb-schema-%s-to-uyuni-reportdb-schema-%s" % (  #  pylint: disable=consider-using-f-string
            last_version,
            new_version,
        )
        pcom.append(fakedir)
...
def run_upgrade(upgrade_script, new_version):
    ...
    command = (
        "export SUMA_TEST_SCHEMA_VERSION=%s; export PERLLIB=%s; %s -y --reportdb"  #  pylint: disable=consider-using-f-string
        ...
        for element in sorted(os.listdir(log_path)):
            if re.search(r"^schema-from-.*\.log$", element):
                ...
                with open(log_path + "/" + element) as file:  #  pylint: disable=unspecified-encoding
                    for line in file.read().splitlines():
                        print(line)
        print("Failed command: {}".format(command))  #  pylint: disable=consider-using-f-string
        raise RuntimeError(
