...
Feature: Disable and re-enable monitoring of the server
  ...
  Scenario: Check that monitoring is disabled
    ...
    And file "/etc/tomcat/conf.d/tomcat_java_jmx.conf" should not exist on server
    And file "/etc/sysconfig/rhn/taskomatic_jmx.conf" should not exist on server
  ...
  Scenario: Check that monitoring is enabled
    ...
    And file "/etc/tomcat/conf.d/tomcat_java_jmx.conf" should contain "jmx_prometheus_javaagent.jar=5556" on server
    And file "/etc/sysconfig/rhn/taskomatic_jmx.conf" should contain "jmx_prometheus_javaagent.jar=5557" on server
    And port "3333" should be closed
    And port "3334" should be closed
