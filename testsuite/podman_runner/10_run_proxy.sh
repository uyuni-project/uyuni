...
create_proxy_configuration() {
  cat <<EOF > /tmp/test-all-in-one/config.yaml
...
EOF
  ...
  cat <<EOF > /tmp/test-all-in-one/httpd.yaml
httpd:
...
EOF
  ...
  cat <<EOF > /tmp/test-all-in-one/ssh.yaml
ssh:
  server_ssh_key_pub: |
  ...
  server_ssh_push: |
$(sed 's/^/    /' /tmp/test-all-in-one/id_openssh_rsa)
  server_ssh_push_pub: |
$(sed 's/^/    /' /tmp/test-all-in-one/id_openssh_rsa.pub)
EOF
}
...
run_proxy_containers() {
  sudo --login podman run \
    ...
    --volume /tmp/test-all-in-one/:/etc/uyuni \
    --name proxy-httpd \
