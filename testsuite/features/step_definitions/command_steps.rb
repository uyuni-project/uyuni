...
Then(/^podman container "([^"]*)" should be (running|healthy) on "([^"]*)"$/) do |container, state, host|
  ...
  case state
  when 'running'
    node.run("podman inspect --format '{{.State.Running}}' #{container} | grep -x true", check_errors: true, verbose: true)
  when 'healthy'
    node.run("podman inspect --format '{{if .State.Health}}{{.State.Health.Status}}{{end}}' #{container} | grep -x healthy", check_errors: true, verbose: true)
  else
