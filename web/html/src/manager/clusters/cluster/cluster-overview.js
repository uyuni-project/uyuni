            ...
            {fetchingProps ? 
              ...
              <>
                ...
                <PanelRow>
                  ...
                  <SystemLink id={cluster.managementNode.id} className='col-md-6'>{cluster.managementNode.name}</SystemLink>
                </PanelRow>
                <PanelRow>
                  <Label name={t('System group')} className='col-md-3'/>
                  <SystemGroupLink id={cluster.group.id} className='col-md-6'>{cluster.group.name}</SystemGroupLink>
                </PanelRow>
              </>
            }
        </Panel>
        ...
        <Table
            ...
            <Column
                ...
                cell={(row : ClusterNodeType) =>
                    <input type="checkbox" value={row.hostname} 
                        name="nodes"
                        checked={selections.has(row.hostname)}
                        onChange={(ev: SyntheticInputEvent<HTMLInputElement>) => onSelectNode(ev, row.hostname)} />                
                }
            />
            <Column
                columnKey="hostname"
            ...
            <Column
                ...
                width="15%"
                header={t('System')}
                cell={(row : ClusterNodeType) => row.server ? <SystemLink id={row.server.id}>{row.server.name}</SystemLink>: t("(none)") }
            />
            {
