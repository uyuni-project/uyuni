...
$PODMAN_CMD run --pull missing --privileged -d --network network -p 9092:9090 -v /tmp/testing/rh_product_uuid:/sys/class/dmi/id/product_uuid -v /tmp/testing:/tmp -v ${src_dir}/testsuite/podman_runner/salt-minion-entry-point.sh:/salt-minion-entry-point.sh --name rhlike_minion -h rhlike_minion ghcr.io/$UYUNI_PROJECT/uyuni/ci-test-rocky-minion:$UYUNI_VERSION bash -c "exec tail -f /dev/null"
...
$PODMAN_CMD exec -d rhlike_minion bash -c "/salt-minion-entry-point.sh server 1-RH-LIKE-KEY"
