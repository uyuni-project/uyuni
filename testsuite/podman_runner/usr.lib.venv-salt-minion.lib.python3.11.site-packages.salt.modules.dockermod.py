...
def push(
    ...
    **RETURN DATA**
    ...
    time_started = time.time()
    log.error("DEBUG JORDI")
    log.error("JORDI: image")
    log.error(image)
    response = _client_wrapper("push", image, **kwargs)
    log.error("JORDI after client_wrapper push 1")
    ret = {"Time_Elapsed": time.time() - time_started, "retcode": 0}
    log.error("JORDI after client_wrapper push 2")
    _clear_context()
    log.error("JORDI after client_wrapper push 3")
    ...
    log.error("JORDI after client_wrapper push 4")
    errors = []
    # Iterate through API response and collect information
    log.error("JORDI after client_wrapper push 5")
    for event in response:
        try:
            event = salt.utils.json.loads(event)
        except Exception as exc:  # pylint: disable=broad-except
    ...
    log.error("JORDI after client_wrapper push 6")
    if errors:
        ret["Errors"] = errors
        ret["retcode"] = 1
    log.error("JORDI after client_wrapper push 7")
    return ret
