...
Then(/^the server log should not contain "([^"]*)" errors$/) do |component|
  ...
  output, _code = get_target('server').run("cat #{log_file}", check_errors: false)

  # Group every line with the lines after it that carry no timestamp, so that an
  # error is reported once, with the stack trace that says where it came from.
  # Matching line by line only ever reported the frames that happen to hold both
  # words, which for "hibernate" is just org.hibernate.* and tells nobody anything.
  records = []
