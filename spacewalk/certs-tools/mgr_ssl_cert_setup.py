...
def processCommandline():
     ...
     parser.add_argument('-i', '--intermediate-ca-file', action='append', default=[],
                         help='Path to an intermediate CA')
     parser.add_argument('-s', '--server-cert-file', help='Path to the Server Certificate')
     parser.add_argument('-k', '--server-key-file', help='Path to the Server Private Key')
...
def checkOptions(options):
     ...
     if not os.path.exists(options.server_cert_file):
         log_error("Server Certificate: file not found {}".format(options.server_cert_file))
         sys.exit(1)
     ...
     if not os.path.exists(options.server_key_file):
         log_error("Server Private Key: file not found {}".format(options.server_key_file))
         sys.exit(1)
...
def isCA(cert):
    out = subprocess.run(["openssl", "x509", "-noout", "-ext", "basicConstraints", "-in", cert],
            stdout=subprocess.PIPE, stderr=subprocess.PIPE)
    if out.returncode:
        log_error("Unable to parse the certificate: {}".format(out.stderr.decode("utf-8")))
        return False
...
def getCertData(cert):
    data = dict()
    out = subprocess.run(["openssl", "x509", "-noout", "-subject", "-subject_hash", "-startdate", "-enddate", "-issuer", "-issuer_hash", "-modulus", "-in", cert],
            stdout=subprocess.PIPE, stderr=subprocess.PIPE)
    if out.returncode:
        log_error("Unable to parse the certificate: {}".format(out.stderr.decode("utf-8")))
        return None
...
def getCertWithText(cert):
    out = subprocess.run(["openssl", "x509", "-text", "-in", cert],
            stdout=subprocess.PIPE, stderr=subprocess.PIPE)
    if out.returncode:
...
def getRsaKey(key):
    ...
    out = subprocess.run(["openssl", "rsa", "-passin", "pass:invalid", "-in", key],
            stdout=subprocess.PIPE, stderr=subprocess.PIPE)
    if out.returncode:
...
def checkKeyBelongToCert(key, cert):
    out = subprocess.run(["openssl", "rsa", "-noout", "-modulus", "-in", key],
            stdout=subprocess.PIPE, stderr=subprocess.PIPE)
    if out.returncode:
        log_error("Invalid RSA Key: {}".format(out.stderr.decode("utf-8")))
        raise CertCheckError("Invalid Key")
    keyModulus = out.stdout.decode("utf-8")
    out = subprocess.run(["openssl", "x509", "-noout", "-modulus", "-in", cert],
            stdout=subprocess.PIPE, stderr=subprocess.PIPE)
    if out.returncode:
...
def checkCompleteCAChain(workdir, certData):
    ...
    isValid(certData[serverCertHash]['startdate'], certData[serverCertHash]['enddate'], certData[serverCertHash]['subject'])
    ...
    if not foundRootCA:
        raise CertCheckError("Incomplete CA Chain. Unable to find issuer of '{}'".format(subject))


def generateJabberCert(options, workdir, certData):
    certWithChain = generateCertWithChainFile(os.path.join(workdir, SRV_CERT_NAME), workdir, certData)
...
def checks(options, workdir, certData):
    ...
    checkKeyBelongToCert(os.path.join(workdir, SRV_KEY_NAME), os.path.join(workdir, SRV_CERT_NAME))
