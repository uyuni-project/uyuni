...
When(/^I refresh the packages on "([^"]*)" through XML-RPC$/) do |host|
  ...
  node_id = CommonLib.retrieve_server_id(node.full_hostname)
  now = Time.now
...
When(/^I reboot "([^"]*)" through XML-RPC$/) do |host|
  ...
  check_restart(node.full_hostname, reboot_timeout)
  ...
  @client_api.call('schedule.list_failed_actions', @sid).each do |action|
    ...
    raise(StandardError, 'System not found') if systems.all? { |system| system['server_id'] == node_id }
  end
end
