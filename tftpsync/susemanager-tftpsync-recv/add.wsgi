...
def application(environ, start_response):
    ...
    if not (CFG.TFTPBOOT and re.match('^/[\w]+.*$', CFG.TFTPBOOT) and
        ...
        logger.error("Insecure directory [%s]." % directory)
        content = 'Insecure directory'
    elif ".." in file_name or not re.match('^[\:a-zA-Z0-9._-]+$', file_name):
        logger.error("Insecure file_name [%s]." % file_name)
        content = 'Insecure file_name'
    elif not (CFG.RHN_PARENT_IP and CFG.PROXY_IP and CFG.RHN_PARENT and CFG.PROXY):
        ...
        try:
            ...
            if file_type == 'pxe' or file_type == 'grub':
                ...
                file_content = file_content.replace(CFG.RHN_PARENT_IP, CFG.PROXY_IP)
                file_content = file_content.replace(CFG.RHN_PARENT, CFG.PROXY)
                tf.write(file_content)
                tf.close()
                os.rename(tfname, rfname)
                if not isinstance(tfpointer, OutputType):
    ...
    # remove tmp file if exists
    if tfpointer and hasattr(tfpointer, "name") and os.path.exists(tfpointer.name):
        os.unlink(tfpointer.name)
