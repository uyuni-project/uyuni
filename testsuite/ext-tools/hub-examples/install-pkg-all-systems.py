...
if successfulResponses["Responses"]:
    systemsPerServer = {
        server_id: [item["id"] for item in response]
        for server_id, response in zip(
            successfulResponses["ServerIds"], successfulResponses["Responses"]
        )
    }
    for serverId, systemIds in systemsPerServer.items():
        print(
            f"Unicast call to {serverId} server, to schedule a install of package ID {args.packageId} into {systemIds} system"
        )
        result = client.unicast.system.schedulePackageInstall(
            hubSessionKey, serverId, systemIds, [args.packageId], datetime.now()
        )
