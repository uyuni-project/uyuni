...
function toggleRowVisibility(id, rowHash) {
  if (!rowHash[id]) { return; }
  if (!rowHash[id].hasChildren) { alert("No Kids");return; }
  rowHash[id].toggleVisibility();
  return;
}

function showAllRows(rowHash) {
  var row;
  for (var i in rowHash) {
    row = rowHash[i];
    if (!row) { continue; }
...
function Row(cells, image, columns) {
...
// we start with columnsPerRow, because we want to skip the td cells of the parent tr.
    for (var i = this.columnsPerRow; i < this.cells.length; i++) {
...
function createParentRows(channelTable, rowHash, columnsPerRow) {
  for (var i = 0; i < channelTable.rows.length; i++) {
    ...
    if (isParentRowNode(tableRowNode)) {
      ...
      if (!image) { continue; }
      rowHash[id] = new Row(cells, image, columnsPerRow);
    }
  }
  return;
}
