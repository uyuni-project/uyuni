...
sudo -i podman run --privileged --rm -d --network network -p 9092:9090 -v /tmp/rh_product_uuid:/sys/class/dmi/id/product_uuid -v /tmp/testing:/tmp -v ${src_dir}/testsuite/podman_runner/salt-minion-entry-point.sh:/salt-minion-entry-point.sh --name rhlikeminion -h rhlikeminion ghcr.io/$UYUNI_PROJECT/uyuni/ci-test-rocky-minion:$UYUNI_VERSION bash -c "/salt-minion-entry-point.sh server 1-RH-LIKE-KEY"

# sleep 10
sudo -i podman exec rhlikeminion bash -c "ssh-keygen -A && /usr/sbin/sshd -e"
sudo -i podman exec rhlikeminion bash -c "if [ ! -d /root/.ssh ];then mkdir /root/.ssh/;chmod 700 /root/.ssh;fi;cp /tmp/authorized_keys /root/.ssh/"
