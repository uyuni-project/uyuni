...
LOOKUP_SERVER_ARCH('armhf-debian-linux'), LOOKUP_PACKAGE_ARCH('arm-deb'), 0 from dual
...
insert into rhnServerPackageArchCompat (server_arch_id, package_arch_id, preference) select
LOOKUP_SERVER_ARCH('armhf-debian-linux'), LOOKUP_PACKAGE_ARCH('src-deb'), 1000 from dual
where not exists (select 1 from rhnServerPackageArchCompat where server_arch_id = LOOKUP_SERVER_ARCH('armhf-debian-linux') and package_arch_id = LOOKUP_PACKAGE_ARCH('src-deb'));
