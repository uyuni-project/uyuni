...
/**
 * This class represents the final step in the registration process for SCC systems on behalf of peripheral servers
 * in a hub-peripheral scenario,
 * It analyzes which systems were successfully registered and updates the proxy records accordingly.
 */
public class SCCProxyRegistrationUpdateProxyRecords implements SCCSystemRegistrationContextHandler {
    private static final Logger LOG = LogManager.getLogger(SCCProxyRegistrationUpdateProxyRecords.class);
...
//        context.getItems().forEach(cacheItem -> cacheItem.getOptServer().ifPresent(ServerFactory::save));
    ...
    protected void updateSuccessfullyRegisteredItems(SCCSystemRegistrationContext context) {
        for (SCCSystemCredentialsJson systemCredentials : context.getProxyRegisteredSystems()) {
