...
public class ScheduleHandlerTest extends BaseHandlerTestCase {
    ...
    public void testCannotCancelPendingActionsWithPrerequisite() throws Exception {
        ...
        Action parent = ActionFactoryTest.createNewAction(admin, ActionFactory.TYPE_PACKAGES_UPDATE);
        ServerActionTest.createServerAction(server, parent);

        Action child = ActionFactoryTest.createNewAction(admin, ActionFactory.TYPE_SCRIPT_RUN);
        ...
        try {
            ...
            if (!(ex instanceof UnsupportedOperationException)) {
                fail("An UnsupportedOperationException should have been thrown. " +
                    ex.getClass() + " was thrown instead");
            }

            assertEquals("Cannot cancel an action with a pending prerequisite.", ex.getMessage());
        }
    }
    ...
    public void testCannotCancelPickedUpAction() throws Exception {
        ...
        Action action = ActionFactoryTest.createNewAction(admin, ActionFactory.TYPE_PACKAGES_UPDATE);
        ...
        try {
            ...
            assertEquals("Cannot cancel an action in PICKED UP state.", ex.getMessage());
        }
    }
