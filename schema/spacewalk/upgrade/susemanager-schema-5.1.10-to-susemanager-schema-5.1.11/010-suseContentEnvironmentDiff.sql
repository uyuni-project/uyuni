...
CREATE TABLE IF NOT EXISTS suseContentEnvironmentDiff(
    ...
    modified            TIMESTAMPTZ
                            DEFAULT (current_timestamp) NOT NULL,
    CONSTRAINT suse_ced_pid_sid_tid_uq UNIQUE(project_id, env_id, channel_id, entry_id, entry_type)
);

CREATE INDEX IF NOT EXISTS suse_ced_t_id_idx
    ON suseContentEnvironmentDiff(entry_type, entry_id);

CREATE INDEX IF NOT EXISTS suse_ced_nvt_idx
    ON suseContentEnvironmentDiff(entry_name, entry_version, entry_type);

CREATE INDEX IF NOT EXISTS suse_ced_cid_idx
