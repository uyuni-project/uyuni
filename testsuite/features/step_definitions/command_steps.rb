...
When(/^I sync channel "([^"]*)"$/) do |channel|
  $command_output, _code = get_target('server').run("mgr-sync add channel #{channel}", check_errors: false, verbose: true)
end
...
When(/^I sync all channels for "([^"]*)", synchronizing only client tools dependencies$/) do |product_os_version|
  ...
  channels_to_synchronize.each do |channel|
    ...
    $command_output, _code = get_target('server').run("mgr-sync add channel #{channel} --metadata-only", check_errors: false, verbose: true)
