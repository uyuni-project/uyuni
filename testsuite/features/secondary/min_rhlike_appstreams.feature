...
Feature: Native AppStreams support for Red Hat-like Salt minion
  ...
  Scenario: Verify that channel AppStreams are available for the system
    ...
    Then I should see "andromeda:2.1-cbox" as unchecked
    And I should see "andromeda:2.0-cbox" as unchecked

  Scenario: Ensure only the non-modular package is available when AppStreams are disabled
    Given I am on the Systems overview page of this "rhlike_minion"
    And I follow "Software" in the content area
    And I follow "Install"
    Then I should see a "andromeda-dummy-1.0" text
    And I should not see a "andromeda-dummy-2.0" text
    And I should not see a "andromeda-dummy-2.1" text

  Scenario: Install non-modular package
    When I install package "andromeda-dummy=1.0-1.1" on this "rhlike_minion"
  ...
  Scenario: Verify that modular packages do not appear as upgrade options when AppStreams are disabled
    ...
    Then I should not see a "andromeda-dummy-2" text

  Scenario: Enable AppStream module
    Given I am on the Systems overview page of this "rhlike_minion"
    And I follow "Software" in the content area
    And I follow "AppStreams"
    And I check "andromeda:2.0-cbox"
    ...
    And I wait until event "Change AppStreams (enable: [andromeda:2.0]) scheduled by admin" is completed
  ...
  Scenario: Upgrade to modular package
    ...
    And I follow "Packages"
    And I follow "Upgrade"
    Then I wait until I see "andromeda-dummy-2.0" text, refreshing the page
    And I should see a "andromeda:2.0" text
    ...
    Then "andromeda-dummy-2.0" should be installed on "rhlike_minion"
  ...
  Scenario: Change enabled AppStream module
    ...
    And I follow "Software" in the content area
    And I follow "AppStreams"
    And I check "andromeda:2.1-cbox"
    And I click on "Apply Changes (2)"
    Then I should see a "Changes Summary" text
    When I click on "Confirm"
    Then I should see a "Updating the selection of AppStream modules has been scheduled." text
    And I wait until event "Change AppStreams (disable: [andromeda:2.0] enable: [andromeda:2.1]) scheduled by admin" is completed

  Scenario: Ensure only the correct modular package is available and install it
    Given I am on the Systems overview page of this "rhlike_minion"
    And I follow "Software" in the content area
    And I follow "Install"
    Then I should see a "andromeda-dummy-2.1" text
    And I should see a "andromeda:2.1" text
    And I should not see a "andromeda-dummy-2.0" text
    And I should not see a "andromeda-dummy-1.0" text
    When I check "andromeda-dummy-2.1" in the list
    And I click on "Install Selected Packages"
    And I click on "Confirm"
    Then I should see a "1 package install has been scheduled" text
    When I wait until event "Package Install/Upgrade scheduled" is completed
    Then "andromeda-dummy-2.1" should be installed on "rhlike_minion"
  ...
  Scenario: Cleanup: remove modular package
    ...
    And I enter "andromeda-dummy" as the filtered package name
    And I click on the filter button
    And I check "andromeda-dummy-2.1" in the list
    And I click on "Remove Packages"
    And I click on "Confirm"
    Then I should see a "1 package removal has been scheduled" text
    When I wait until event "Package Removal scheduled" is completed
  ...
  Scenario: Cleanup: disable AppStream module
    ...
    And I follow "Software" in the content area
    And I follow "AppStreams"
    And I uncheck "andromeda:2.1-cbox"
    And I click on "Apply Changes (1)"
    Then I should see a "Changes Summary" text
    When I click on "Confirm"
    Then I should see a "Updating the selection of AppStream modules has been scheduled." text
    And I wait until event "Change AppStreams (disable: [andromeda:2.1] ) scheduled by admin" is completed
