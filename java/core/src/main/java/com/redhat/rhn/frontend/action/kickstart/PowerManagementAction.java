...
public class PowerManagementAction extends RhnAction {
            ...
            if (context.wasDispatched("kickstart.powermanagement.jsp.remove.cobblerprofile")) {
                ...
                if (error == null) {
                    ...
                    if (context.wasDispatched("kickstart.powermanagement.jsp.power_on")) {
                        error = handlePowerOn(request, user, server, sid);
                    }
                    if (context.wasDispatched("kickstart.powermanagement.jsp.power_off")) {
                        error = handlePowerOff(request, user, server, sid);
                    }
                    if (context.wasDispatched("kickstart.powermanagement.jsp.reboot")) {
                        error = handleReboot(request, user, server, sid);
                    }
    ...
    private ValidatorError handlePowerOn(HttpServletRequest request, User user, Server server, Long sid) {
        ValidatorError error = new CobblerPowerCommand(user, server, Operation.POWER_ON).store();
        if (error == null) {
    ...
    private ValidatorError handlePowerOff(HttpServletRequest request, User user, Server server, Long sid) {
        ValidatorError error = new CobblerPowerCommand(user, server, Operation.POWER_OFF).store();
        if (error == null) {
    ...
    private ValidatorError handleReboot(HttpServletRequest request, User user, Server server, Long sid) {
        ValidatorError error = new CobblerPowerCommand(user, server, Operation.REBOOT).store();
        if (error == null) {
