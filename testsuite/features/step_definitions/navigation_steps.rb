...
Then(/^I should not see a "(.*)" text in the content area$/) do |text|
  within('#spacewalk-content') do
    raise(StandardError, "Text #{text} found") unless has_no_content?(text)
  end
end
...
When(/^I wait at most (\d+) seconds until I do not see "([^"]*)" text, refreshing the page$/) do |seconds, text|
  ...
  CommonLib.repeat_until_timeout(message: "I still see text '#{text}'", timeout: seconds.to_i) do
...
Then(/^I should see a "([^"]*)" text or "([^"]*)" text$/) do |text1, text2|
  raise(StandardError, "Text '#{text1}' and '#{text2}' not found") unless has_content?(text1) || has_content?(text2)
end
...
Then(/^I should see a text like "([^"]*)"$/) do |title|
  raise(StandardError, "Regular expression '#{title}' not found") unless has_content?(Regexp.new(title))
end
...
Then(/^I should not see a "([^"]*)" text$/) do |text|
  raise(StandardError, "Text '#{text}' found on the page! FAIL") unless has_no_content?(text)
end
