...
module TwoPenceLib
  ...
  def twopence_init_common_nodes
  ...
  def extend_nodes
    $nodes.each do |node|
      ...
      raise(ScriptError, "Cannot connect to get hostname for '#{$named_nodes[node.hash]}'. Error: #{code}") if code.nonzero?
      ...
      puts("Host '#{$named_nodes[node.hash]}' is alive with hostname #{hostname.strip} and FQDN #{fqdn.strip}") unless $build_validation
    end
  end

  # rubocop:disable Metrics/MethodLength
  # Generate two hash table of node by host and viceversa
  def hash_tables_host_node
    $node_by_host = {
      'localhost' => $localhost,
      'server' => $server,
      'proxy' => $proxy,
  ...
  def client_public_ip(host)
    ...
    interface =
      ...
      when 'kvm_server', 'xen_server'
        ...
        raise(ArgumentError, "Unknown net interface for #{host}")
      end
    node.init_public_interface(interface)
    output, code = node.run("ip address show dev #{interface} | grep 'inet '")
    raise(ArgumentError, 'Cannot resolve public ip') unless code.zero?
