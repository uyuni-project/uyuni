...
sudo -i podman exec uyuni-server-all-in-one-test bash -c "ssh-keygen -A && /usr/sbin/sshd -e && eval $(ssh-agent) && ssh-add ssh_host_rsa_key && cp ssh_host_rsa_key.pub /tmp"
