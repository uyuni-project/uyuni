...
Feature: Install a package on the Debian-like minion with staging enabled
  ...
  Scenario: Pre-requisite: install virgo-dummy-1.0 package, make sure orion-dummy is not present on Debian-like minion
    ...
    And I run "apt update" on "deb_minion"
    And I remove package "orion-dummy" from this "deb_minion"
  ...
  Scenario: Pre-requisite: refresh package list on Debian-like minion
    When I refresh packages list via spacecmd on "deb_minion"
    And I wait until refresh package list on "deb_minion" is finished
    Then spacecmd should show packages "virgo-dummy-1.0" installed on "deb_minion"
  ...
  Scenario: Install package in the future and check for staging on Debian-like minion
    ...
    And I wait until the package "orion-dummy_1.1" has been cached on this "deb_minion"
    And I wait for "orion-dummy-1.1" to be installed on "deb_minion"
