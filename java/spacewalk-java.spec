...
%if 0%{suse_version}
echo "wrapper.java.classpath.61=/usr/share/java/slf4j/api.jar
wrapper.java.classpath.62=/usr/share/java/slf4j/jcl.jar" >> conf/default/rhn_taskomatic_daemon.conf
%endif
%endif
...
- (temporalily) removing stuff preventing build in upstream koji
- refactor doAfterBody in different sub-methods to comply with checkstyle.
- invalid jsp syntax
- checkstyle fixes
- Fix add/remove to SSM icons and translation
- Port images to font iconts (FontAwesome)
- Use alternate quotes. HTML parsers choke on nested quotes without escaping.
- To avoid losing the image alternate, add iconAlt.
- Jumbotron replaced for Panel
- uniqueList is lazy and starts as null, use the getter which does the lazy
  initialization.
- This is the reason decorators could not find the ListCommand. Using the wrong
  Id. No need to initialize with a initial command.
- Check style fixes
- 1 class modified to give more margin and 1 icon replaced
- Panels and tag Small added
- icons changed
- class added to main menu to make it responsible
...
- Use the toolbat jsp tag
- Fix toolbar: either icon or (back-compat) img are needed, but not both
- TB3: Audit/OpenScap form
- TB3: Delete organisation
- Fully responsive
- Fix the old List not creating certain element blocks breaking the layout: -
  if there is no alphabar we still render the alphabar container if there is
  other top-addon-content
- move the semantics of id and style of the List tag to the div and not the
  inner table
- set the current list for additional decorators
- start with an initial ListCommand
- Don't show the top addons in the list if they have no content as the addons
  have padding.
- remove xhtml tags
- icon added to the title
- TB3: Admin/Users confirmations
- Panels added
- TB3: Address tag
- panels class added, tables inline code removed
- Remove self-closing paragraphs. Not valid in html5. Remove self closing
  anchors. Replace them with closed anchors.
- hidden element does not exist
- TB3: Admin/Subsriptions/Entitlements/Organizations layout
- Javascript added to recognize if a panel is dissabled and hidden by the user
- Javascript added to recognize if a panel is dissabled and hidden by the user
- Adding the rhn:toolbar icons.
- port to FontAwesome 4.x
- panels, buttons and icons styles added
- TB3: Configuration/Add files/Create File form
- TB3: Configuration edit channel properties form
- class lead removed
- Revert "Changed <table class="details"> to <table class="table">"
- TB3: Monitoring/Probe suite/create or edit
- TB3: Configuration/channel/channel create form
- Zoom scale 1 for mobiles. empty spaces deleted. Panels added.
- TB3: Software/Packages/Details/Overview
- changed <div align="right"> to <div class="text-right">
- Changed <table class="details"> to <table class="table">
- More icons added
- package icon added
- Icons fontawesome added
- Overview page formated
- replace a &nbsp; with some padding
- the half-table can't be removed as it determines how the footer message looks
  like.
- use the bootstrap classes, even if these spans do not feel right
- TB3: Channels/channel details presentation form
- Rename check_all.js to spacewalk-checkall.js
- buttons and panel style added
- Colour added to icons in legends
- Fix typo in comment
- Panels added and tables well formated
- Use alert-info and alert-warning classes correctly
- Bootstrapify all obsoleted site-alerts
- Adjust column sizes
- Panels + tables formated
- use a panel for makeEmptyTable
- TB3: Errata update form
- fix empty lists
- Port the old list and unify more code with UnpagedListDisplay
- New list structure based on Cynthia's mockup. Updated decorator and its
  documentation to reflect the structure. Also updated
  https://fedorahosted.org/spacewalk/wiki/UserInterface
- Refactor UnpagedListDisplayTag and ListDisplayTag so that all the copy pasted
  code is now common in ListDisplayTagBase.
- port the old list tag to the new structure
- Buttons and icons in the title fixed
- Panel, form and table style applied
- Tables into Panels and <i> tag fixed
- All page formated with panels and tables from Bootstrap
- Rewrote spacewalk-login.js to make focus.js obsolete
- Adjust the column sizes
- Add the form-control class to all text inputs
- Add missing <div id="images-table">
- Table replaced to Panel. Form classes applied
- TB3: Virtualization images form
- Panels included. empty div tags deleted. Icons included. obsolete classes
  removed
- th sintax error fixed. Icon added. Font tag changed to P
- th sintax error fixed. Icon added. Font tag changed to P
- Make display.js obsolete by calling jquery directly
- Refactor images.js to spacewalk-images.js
- TB3: Create new erratum form
- TB3: Delete software channel form
- TB3: Systems/Kickstart/Profiles/ new or upload forms
- Schedule section (all pages) have been correctly formated. Divs have been
  deleted, tables well formated and buttons classes applied
- buttons and panels added. Tables correctly formated
- Call create_checkall_checkbox() when document is ready
- put the decorators in the right hooks
- buttons with class success
...
- convert the tasks section of the overview in a panel
- panels added for forms
- Panels added to forms inside the Your Account section
- TB3: Systems/Activation Keys/ ranking, deletion and packages forms
- Generate another div element inside the filter-input
- TB3: Systems/Activation keys/Child channels form
- panel-title added to the titles in the panels
- Locale Preferences: Panels added
- TB3: Systems/Activation Keys/Create Activation Key form
- clear-btn fixed
- icon added to the button Update Email in user account
- changes in language files to add icons in some inline code
- systems icons changed
- TB3: Channels/Software Channels/Repositories/new repo form
- Bug fixed: Glyphicons are not allowed - use only FontAwesome icons
- TB3: Forgot a colon for the control label.
- TB3: Channels/Manage Channels/Create channel form
- Changed users/organizations icons.
- Entitlements icon changed.
- Updated Admin/Spacewalk Configuration info icons.
- Update icons of Configuration/Configuration channels.
- Updated channel icons.
- Fix ISE when changing user details on /rhn/users/UserDetails.do
- Changed Kickstart icons.
- Changed a few more icons.
- Put the summary in the right place on Credentials page
- TB3: Channels/Distribution Channel Mapping/Create new distromap form
- TB3: Channels/Package search form
- Changed a few icons
- perl: port the toolbar move the pull-right for the links to the css as a
  mixin (DRY)
- Language files changed: they have the style of Vew System Groups inside
- New styles applied to the main objects in System Overview
- Legends titles and colour changed
- I had already renamed clearbtn to clear-btn in the css
- Fix the System Set counter in the Perl pages.
- Make overview legend heading the same size as the other legends
- initial port of the perl pages to the new layout/css
- .wrap is not global, does not include footer: rename wrap -> spacewalk-top-
  wrap
- Go back to loading check_all.js in <head>
- paginator styled and icon fixed in systems legend
- TB3: Admin/Task Schedules/Schedule task form
- Require "development_environment = 1"
...
- enable less.js debug/error messages
- Re-order javascript references
- Provide a static version of less version 1.4.1
- TB3: Admin/ISS Configuraion (all)
- spacewalk-bar pushed to the right
- TB3: Admin/Config/Boostrap form
- Remove obsolete inline script (replaced by code in spacewalk-login.js)
- TB3: Admin/Configuration/Certificate form
- This is loaded in the footer, loading it once should be enough
- Move content of render.js into spacewalk-essentials.js
- Move reference to spacewalk-essentials.js back to the head
- Include a static version of bootstrap.js (v3.0.0)
- Remove obsolete type="text/javascript"
- Load spacewalk-essentials in the footer
- Serve jquery locally instead of fetching it from a CDN
- fix empty lists closing tags
- The table heading of inactive systems is really the title.
- rl:list re-arrangement title goes into the panel heading but filter/addons
  stay as a thead links go into a footer but pagination/etc go into a tfoot
- Refactor the List code so that code is not split inconsistently across
  methods leaving tags open and having boolean state everywhere. Some repeated
  writing of html is not bad if one can read the code without jumping from
  method to method remembering the state of a open tag.
- align the csv link to the right. May be we make it a button?
- Port most of the list widgets to output html that can be better styled with
  bootstrap.
- port the alphabar to twitter bootstrap
- TB3: Spacewalk Configuration: General, Monitoring forms
- TB3: Admin/Organizations/org/Subscriptions/System Entitlements form
- Fixed Uncaught SyntaxError: Unexpected token )
- Cleaned up images.jsp
- TB3: Admin/Organizations/org/Details form
- Systems selected
- TB3: Admin/Organizations/Create New Organization
- Jquery files for login, UL lists and SSM toolbar
- Use if .. else .. for better readability
- port to JQuery /rhn/audit/Search.do
- Port to JQuery: Javascript snippet that prevents form submission with ENTER
- Use icons instead of images
- port to JQuery
- add jquery as a replacement for prototypewq
- Legends
- TB3: Admin/Organizations/Organization Configuration form
- Remove trailing whitespace
- Make the footer stick to the bottom even if there is no content
- TB3: Monitoring/Probe Suites/Edit Probe form
- TB3: Ranking channels form
- Fix markup of the SSM system selection counter
- TB3: Added style to dropdown on Wizard profile, step 1.
- TB3: Systems/Custom System Info/Edit custom key form
- TB3: Systems/Kickstart/Create GPG key form
- TB3: Systems/Kickstart Snippets/Create-Edit form
- TB3: Provisioning/preservation form
- TB3: Systems/Kickstart/Distributions form
- TB3: Systems/Kickstart/GPG and SSL Keys form
- TB3: Kickstart wizard profile, first step.
- TB3: Systems/Advanced Search form
- TB3: System group create form.
- Move spacewalk-bar inside the <nav> in header.jsp
- TB3: Create monitoring probe form
- TB3: Systems/Custom Info/Create Key
- TB3: Systems/Note Edit
- TB3: Systems/Details/Hardware
- TB3: Ported upload local file form
- TB3: Ported upload local file form
- Fix single system selection and overview ajax pages: <script> is not a self
  closing tag
- Re-introduce the custom login_banner functionality
- Fix indentation
- Remove xhtml tag on the relogin page
- Removed XHTML definition
- Add a placeholder to the password field
- Center the content on login screen
- Port system-header.jspf custom html to use the available rhn:toolbar as it
  should be.
- add toolbar.create.customdata string
- use a font-awesome icon instead of an icon image
- add the new icon attributes to rhn-taglib.tld
- port copy pasted html to the rhn:toolbar tag
- - add support for icon in addition to an image - render the html the css
  expects
- Use badges for the probes count
- Fix the relogin screen
- Fix and clean up the login screen
- TB3: SUSE Studio credentials form ported
- TB3: Change E-mail form.
- TB3: Ported personal address form
- TB3: Ported form users/EditAddress.do
- TB3: Account info form finished.
- TB3: User info and account info form
- Show the spacewalk-bar only if the user is authenticated
- TB3: Single system remote command form
- TB3: Single system Properties form
- Remove the search button text
- Removed colons before the user and org name
- Fix legend on the Overview page
- use fontawesome icons on the probe list
- Add missing form-control class and a placeholder string
- Move SSM bar outside of the <header>
- typo: it is alert-info
- Use less files for bootstrap and the main css. We need to look at a way to
  precompile them for production.
- Probe list uses the content-nav manually with html and sets the class
  active/no active using controller context variables.
- Port the dialog renderer (aka content-nav) to use nav-tabs
- Refactor nav-spacewalk to spacewalk-main-nav
- Fix footer css classes and remove inline style
- Fix container class
- Remove inline style and clean up css and script elements
- Fix search icon and add class nav-spacewalk to the top navigation
- Fix column classes
- TB3: Filter in ListView for single columns. ToDO: Multiple columns.
- Fix header elements and classes
- Convert div into nav for the navigation on the left side
- add the new Spacewalk branding, commented, so that can be enabled on local
  development.
- Remove unnecessary ids, classes and code
- First version of the top navigation bar
- TB3: Search buttons
- bootstrap 3 does not provide indentation for stacked pills
- add table styles
- Side navigation is now generating unsorted nesten lists.
- Side navigation for Bootstrap 3.0
- use the grid to layout aside and section
- Cleanup the header include. TODO: the action bar has to be moved out of the
  header into the section (main content)
- Use a standard page layout (html5)
