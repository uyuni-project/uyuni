...
class ImageViewPatches extends React.Component {
    ...
    renderType(row) {
        let icon = [<i className={typeIcons[row.type]} title={typeTitles[row.type]}/>];

        for(let k of row.keywords) {
            icon.push(<i className={typeIcons[k]} title={typeTitles[k]}/>);
        }
        ...
        return (
            ...
            >
                <Column
                    ...
                    header={t("Type")}
                    cell={(row, criteria) => this.renderType(row)}
                />
                <Column
                    columnKey="name"
                    comparator={Utils.sortByText}
                ...
                <Column
                    ...
                    comparator={Utils.sortByText}
                    header={t("Synopsis")}
                    cell={(row, criteria) => row.synopsis}
                />
                <Column
                    columnKey="update"
                    comparator={Utils.sortByDate}
                    header={t("Updated")}
                    cell={(row, criteria) => <DateTime time={row.update}/>}
                />
            </Table>
        );
    }
