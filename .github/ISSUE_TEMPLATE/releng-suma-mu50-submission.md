...
- [ ] Increase `web.version` in `web/conf/rhn_web.conf` (`x.y.z+1`) in `Manager-5.0` branch
  - [ ] Check that the sed that replaces the default tag for SUSE Multi-Linux Manager in the `push.sh` script in `uyuni-tools` specifies the same version you added in `web.version`. If the value is outdated, update it with a PR for `uyuni-tools`. For example, see the [PR done](https://github.com/SUSE/uyuni-tools/pull/43) for fixing the default pull tag after the release of SUSE Manager 5.0.2.
