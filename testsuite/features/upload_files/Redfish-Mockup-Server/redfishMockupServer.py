...
class RfMockupServer(BaseHTTPRequestHandler):
    ...
    def try_to_sleep(self, method, path):
        ...
        if self.server.timefromJson:
            ...
            try:
                ...
                logger.info(
                    "Time is not a float value. Sleeping with default response time"
                )
    ...
    def handle_eventing(self, data_received):
        sub_path = self.construct_path(
            "/redfish/v1/EventService/Subscriptions", "index.json"
        )
            ...
            if (
                ...
                for member in sub_payload.get("Members", []):
                    ...
                    if not success:
                        ...
                        if ("Destination" in subscription) and (
                            "EventTypes" in subscription
                        ):
                            logger.info(("Target", subscription["Destination"]))
                            logger.info(
                                (data_received["EventType"], subscription["EventTypes"])
                            )
        ...
        if not success:
            ...
            if (
                (
                    ...
                    and ("MetricReportValues" in data_received)
                )
                or (
                    ("MetricReportName" in data_received)
                ...
                or (
                    ("MetricName" in data_received)
                    and ("MetricValues" in data_received)
                )
            ):
                # If the EventType in the request is one of interest to the subscriber, build an event payload
                expected_keys = [
                ...
                my_name = data_received.get(
                    "MetricName", data_received.get("MetricReportName")
                )
                ...
                event_payload["@odata.context"] = (
                    "/redfish/v1/$metadata#MetricReport.MetricReport"
                )
                event_payload["@odata.type"] = "#MetricReport.v1_0_0.MetricReport"
                event_payload["@odata.id"] = (
                    "/redfish/v1/TelemetryService/MetricReports/" + my_name
                )
                ...
                event_fpath = self.construct_path(
                    event_payload["@odata.id"], "index.json"
                )
                ...
                if event_payload["@odata.id"] not in [
                    ...
                    collection_payload["Members"].append(
                        {"@odata.id": event_payload["@odata.id"]}
                    )
                collection_payload["Members@odata.count"] = len(
                    collection_payload["Members"]
                )
        ...
        # if this location exists in memory or as file
            ...
            if output_data.get("Members") is not None:
                ...
                if top_count < len(my_members):
                    ...
                    query_string = "&".join(
                        ["{}={}".format(k, v) for k, v in query_out.items()]
                    )
        ...
        if data_received is not None:
            ...
            # Actions framework
                ...
                # SubmitTestMetricReport
                    ...
                    "TelemetryService/Actions/TelemetryService.SubmitTestMetricReport"
                    in self.path
                ):
                ...
                # All other actions (no data checking or response data)
                    ...
                    fpath = self.construct_path(
                        self.path.split("/Actions/", 1)[0], "index.json"
                    )
                    success, payload = self.get_cached_link(fpath)
                    if success:
                        action_found = False
                            ...
                            for action in payload["Actions"]:
                                if action == "Oem":
                                    for oem_action in payload["Actions"][action]:
                                        if (
                                            payload["Actions"][action][oem_action][
                                                "target"
                                            ]
                                            == self.path
                                        ):
                                    ...
                                    if (
                                        payload["Actions"][action]["target"]
                                        == self.path
                                    ):
                                        action_found = True
                                        if (
                                            "ResetType" in data_received
                                            and "ResetType@Redfish.AllowableValues"
                                            in payload["Actions"][action]
                                        ):
                                            if (
                                                data_received["ResetType"]
                                                not in payload["Actions"][action][
                                            ...
                                            if data_received["ResetType"] in (
                                                "ForceOff",
                                                "GracefulShutdown",
                                            ):
                                                payload["PowerState"] = "Off"
                                            elif data_received["ResetType"] in (
                                                "On",
                                                "ForceOn",
                                            ):
...
def main():
    ...
    parser.add_argument(
        "-D", "--dir", "--Dir", help="path to mockup dir (may be relative to CWD)"
    )
    parser.add_argument(
        "-E",
        "--test-etag",
    ...
    parser.add_argument(
        "-P", "--ssdp", action="store_true", help="make mockup SSDP discoverable"
    )
    ...
    if ssdpStart:
        ...
        mySSDP = RfSSDPServer(
            jsonData,
            "{}{}:{}{}".format(protocol, hostname, port, "/redfish/v1"),
            hostname,
        )
