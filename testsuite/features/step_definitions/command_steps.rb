...
Then(/^I should see the "([^"]*)" paygo products$/) do |paygo_product|
  ...
  raise ScriptError, 'Error : Product list is empty' if output_lines.empty?
...
When(/^the server starts mocking an IPMI host$/) do
  get_target('server').run('mkdir -p /etc/ipmi')
  ...
  get_target('server').run('chmod +x /etc/ipmi/fake_ipmi_host.sh', verbose: true, check_errors: true)
  # Check if ipmi_sim is already running
  if get_target('server').run('pgrep -f ipmi_sim', verbose: false, check_errors: false)[1].zero?
    log 'ipmi_sim is already running; skipping startup.'
  else
    get_target('server').run('nohup ipmi_sim -n > /dev/null 2>&1 & disown')
  end
end
...
When(/^I configure the proxy$/) do
  ...
  answers_filepath = '/tmp/config-answers.txt'
  get_target('proxy').inject(temp_file.path, answers_filepath)
  temp_file.close
  temp_file.unlink

  # perform the configuration
  cmd = "configure-proxy.sh --non-interactive --rhn-user=admin --rhn-password=admin --answer-file=#{answers_filepath}"
  proxy_timeout = 600
...
When(/^I generate the configuration "([^"]*)" of containerized proxy on the server$/) do |file_path|
  if running_k3s?
    ...
    command = 'spacecmd -u admin -p admin ' \
              ...
              "#{get_target('proxy').full_hostname.sub('proxy', 'pod-proxy')} " \
              "#{get_target('server').full_hostname} 2048 galaxy-noise@suse.de " \
