...
Feature: Add the Oracle 9 distribution custom repositories
  In order to use Oracle 9 channels with Red Hat "modules"
  ...
  Scenario: Create CLM filters to remove AppStream metadata from Oracle 9
  ...
  Scenario: Create a CLM project to remove AppStream metadata from Oracle 9
    ...
    And I select "oraclelinux-9 for x86_64" from "selectedBaseChannel"
    # "oraclelinux-9-appstream for x86_64" is already checked
    And I check "Custom Channel for oracle9_minion"
    And I click on "Save"
    Then I should see a "EL9-Manager-Tools-Pool for x86_64 Oracle" text
