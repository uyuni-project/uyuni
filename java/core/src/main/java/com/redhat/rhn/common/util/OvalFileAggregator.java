 ...
 * <p>
 * This class is not thread-safe. Instances keep mutable aggregation states (for example, the
 * aggregate DOM document, internal maps, and the finished flag) and are intended to be used by
...
public final class OvalFileAggregator {
    ...
    OvalFileAggregator(Date timestamp) {
        this.documentBuilderFactory = getDocumentBuilderFactory();

        try {
            aggregate = documentBuilderFactory.newDocumentBuilder().newDocument();
        }
