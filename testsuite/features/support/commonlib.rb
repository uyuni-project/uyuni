...
# @param [String] mgr_server The SUSE Manager server to query ('server', 'server2', 'server3'). Defaults to 'server'.
# @return [String] The system ID.
def get_system_id(node, mgr_server: 'server')
  result = api_client_for(mgr_server).system.search_by_name(node.full_hostname)
...
# @return [Boolean] Whether an upgrade action was actually scheduled (false when the package already has no available update)
def trigger_upgrade(hostname, package, mgr_server: 'server')
  ...
  out, code = get_target(mgr_server).run("spacecmd -u admin -p admin system_upgradepackage #{hostname} #{package} -y", check_errors: false)
  return true if code.zero?
  return false if out.include?('No packages to install')

  raise ScriptError, "FAIL: spacecmd system_upgradepackage #{hostname} #{package} returned status code = #{code}.\nOutput:\n#{out}"
end
