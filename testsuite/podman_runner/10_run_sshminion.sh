...
$PODMAN_CMD run --pull newer --privileged --rm -d --network network -v /tmp/testing/opensuse_ssh_product_uuid:/sys/class/dmi/id/product_uuid -v /tmp/testing:/tmp -e container=podman -e SYSTEMD_SECCOMP=0 --cgroups=no-conmon --name opensusessh -h opensusessh ghcr.io/$UYUNI_PROJECT/uyuni/ci-test-opensuse-minion:$UYUNI_VERSION
