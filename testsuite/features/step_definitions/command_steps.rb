...
When(/^I create channel "([^"]*)" from spacecmd of type "([^"]*)"$/)  do |name, type|
  ...
  out, _code = $server.run(command)
end

Then(/^I update init.sls from spacecmd with content "([^"]*)" for channel "([^"]*)"$/) do |content , label|
  filepath = "/tmp/#{label}"
...
And(/^I schedule apply configchannels for "([^"]*)"$/)  do |host|
  node = get_target(host)
