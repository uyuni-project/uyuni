...
ca_crt: |
$(sed 's/^/  /' /tmp/RHN-ORG-TRUSTED-SSL-CERT)
...
httpd:
  ...
  server_crt: |
$(sed 's/^/      /' /tmp/server.crt)
  server_key: |
$(sed 's/^/      /' /tmp/server.key)
EOF

  ssh-keygen -t rsa -b 4096 -C "salt@uyuni-server-all-in-one-test" -f /tmp/id_openssh_rsa -N ""
...
ssh:
  ...
  server_ssh_push: |
$(sed 's/^/    /' /tmp/id_openssh_rsa)
  server_ssh_push_pub: |
$(sed 's/^/    /' /tmp/id_openssh_rsa.pub)
EOF
}
...
cleanup () {
  rm /tmp/id_openssh_rsa /tmp/id_openssh_rsa.pub /tmp/RHN-ORG-TRUSTED-SSL-CERT /tmp/server.crt /tmp/server.key
}
