...
def __virtual__():
    ...
    if _is_valid_endpoint(api_ret["amazon"], "instance-id"):
        INSTANCE_ID = http.query(
            os.path.join(HOST, AMAZON_URL_PATH, "instance-id"), raise_error=False
        )["body"]
        return True
...
def is_payg_instance():
    ...
    try:
        result = subprocess.run(
            flavor_check, check=False, stdout=subprocess.PIPE, universal_newlines=True
        ).stdout.strip()
    ...
    if result == "PAYG":
        log.debug(
            "This minion is a PAYG instance. Adding grains: is_payg_instance = True"
        )
