        ...
        with open(csv_file, 'r', encoding='utf-8') as file:
            ...
            for row in reader:
                try:
                    ...
                    scope = row['scope']
                    authorized = row['authorized'].lower() == 't'
                ...
                endpoint_insert = f"""
                    INSERT INTO endpoint (id, class_method, endpoint, http_method, scope, authorized)
                    VALUES (nextval('endpoint_id_seq'), '{class_method}', '{endpoint}', '{http_method}', '{scope}', {authorized})
                    ...
                    check_namespace_exists = f"""
                        ...
                        BEGIN
                            IF NOT EXISTS (SELECT 1 FROM namespace WHERE namespace = '{namespace}' AND access_mode = '{access_mode}') THEN
                                INSERT INTO namespace (id, namespace, access_mode, description)
                                VALUES (nextval('namespace_id_seq'), '{namespace}', '{access_mode}', NULL)
