...
import Network, { jqXHR } from "utils/network";
...
const MonitoringAdmin = props => {
    ...
    const handleResponseError = (jqXHR: jqXHR, arg: string = "") => {
        ...
        changeStatus(enable)
            .then((result: any) => {
                if (result.success) {
                    ...
                    setMessages(
                        MessagesUtils.error(result.message in msgMap ? msgMap[result.message] : result.message)
                    );
                }
            })
                        ...
                        <div className="col-md-8">
                            ...
                            {restartNeeded ? (
                                <div>
                                    ...
                                    {t(
                                        " Tomcat and Taskomatic is needed for the configuration changes to take effect."
                                    )}
                                </div>
                            ) : null}
